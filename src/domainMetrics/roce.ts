...
export type RoceQuery = QuarterlyMetricQuery;

export interface RoceResult extends QuarterlyMetricIdentity, MetricResultMeta {
...
const emptyResult = (symbol: string, dataType: '1' | '2', subsidiaryCompanyId: string, warnings: string[]): RoceResult => ({
  symbol,
  year: null,
  season: null,
  dataType,
...
export const calculateRoce = async (query: RoceQuery): Promise<RoceResult> => {
  ...
  if (!resolvedQuarter) {
    return emptyResult(symbol, dataType, subsidiaryCompanyId, ['查無任何一季資產負債表/損益表都有資料的季度，無法決定要用哪一季計算 ROCE。']);
  }
