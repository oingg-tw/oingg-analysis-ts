...
export const calculateAltmanZScore = async (query: AltmanZScoreQuery): Promise<AltmanZScoreResult> => {
  ...
  const emptyResult = (year: string | null, season: Season | null): AltmanZScoreResult => ({
    symbol,
    year,
    season,
    dataType,
  ...
  if (priceAnchor) {
    ...
    if (marketCap) {
      ...
      // 這是 BVPS 曾經漏過的同一個坑（見 bvps.ts 的 toPerShare 註解）。
      if (totalLiabilities !== null) x4 = toRatio(marketCapValue, Number(totalLiabilities) * 1000);
    } else {
      warnings.push(`查無 ${symbol} 在 ${priceAnchor.date.toISOString().slice(0, 10)} 或之前的股價/股本資料，X4（市值/總負債）無法計算。`);
    }
  }
