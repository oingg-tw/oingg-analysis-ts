...
`Beneish_M_Score`（貝尼許 M 分數）2026-08-25 改歸類到 [`guru/`](guru.md)（大師策略）——公式本身是 8 變量加權會計異常指數，跟 `Altman_Z_Score` 從 `resilience` 移過去是同一個判斷標準（以學者/研究者命名的複合模型，不是單純的財報比率），不算在這一類。這個分類拿掉 `Beneish_M_Score` 之後 `FCF_Yield` 也在 2026-08-30 做完了，目前全數實作完成。
...
`Accruals_Ratio` 分母用 `Accruals_Ratio` taxonomy 原文的「平均總資產」改成**本季期末總資產**——跟 [`turnoverRatio/`](turnoverRatio/) 用期末餘額同一種刻意簡化（避免多查一期資產負債表）。
...
- `FCF_Yield` 直接用「每股 FCF ÷ 股價」，兩者都已經是每股金額（元），不需要像 [`pFcf/`](pFcf/) 那樣重建市值總額、也不需要查流通股數——用 [`../shared/sourceData/marketCap.ts`](../shared/sourceData/marketCap.ts) 新拆出來的 `getStockPriceAsOf`（只查股價，不查股本），比 `getMarketCapAsOf` 少一次查詢。股價基準日邏輯（優先用財報公告日）、覆蓋率判斷（`hasStockPriceCoverage`）都跟 `pFcf/`/`Altman_Z_Score` X4 完全一致。
