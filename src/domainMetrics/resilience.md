# 財務結構、償債安全與破產預警（resilience_and_financial_health）
...
|---|---|---|---|---|
| `Current_Ratio` | 流動比率 | `Current Assets / Current Liabilities` | MRQ, FY | ✅ 已實作 — [`liquidityRatio/`](liquidityRatio/)，`GET /resilience/liquidity-ratio` |
| `Quick_Ratio` | 速動比率 | `(Cash + Marketable Securities + Receivables) / Current Liabilities` | MRQ, FY | ✅ 已實作 — [`liquidityRatio/`](liquidityRatio/)，`GET /resilience/liquidity-ratio`。公式略有差異，見下方說明 |
| `Cash_Ratio` | 現金比率 | `(Cash + Cash Equivalents) / Current Liabilities` | MRQ, FY | ✅ 已實作 — [`liquidityRatio/`](liquidityRatio/)，`GET /resilience/liquidity-ratio` |
| `DE_Ratio` | 負債權益比 | `Total Debt / Shareholders' Equity` | MRQ, FY | ✅ 已實作 — [`deRatio/`](deRatio/)，`GET /resilience/de-ratio`。`Total Debt` 用的是有息負債（短期借款+應付公司債+長期借款），不是總負債 |
| `Debt_to_Assets` | 資產負債率 | `Total Liabilities / Total Assets` | MRQ, FY | ✅ 已實作 — [`debtRatio/`](debtRatio/)，`GET /resilience/debt-ratio` |
| `Net_Debt_to_EBITDA` | 淨負債對 EBITDA 比 | `(Total Debt - Cash) / EBITDA` | TTM, FY | ✅ 已實作 — [`netDebtToEbitda/`](netDebtToEbitda/)，`GET /resilience/net-debt-to-ebitda`（簡單年化/TTM，沒有原始單季版本，見下方說明） |
| `Interest_Coverage` | 利息保障倍數 | `EBIT / Interest Expense` | TTM, FY | ✅ 已實作 — [`interestCoverage/`](interestCoverage/)，`GET /resilience/interest-coverage`（單季/TTM，見下方說明） |

`Altman_Z_Score`（奧特曼 Z 分數）2026-08-24 改歸類到 [`guru/`](guru.md)（大師策略），不算在這一類——雖然公式本身是加權財務比率、性質很像本分類，但跟其他「大師以自己名字提出的複合公式」放在一起比較合理。這個分類目前**全部指標都已實作**（`Current_Ratio`/`Quick_Ratio`/`Cash_Ratio`/`DE_Ratio`/`Debt_to_Assets`/`Net_Debt_to_EBITDA`/`Interest_Coverage`）。
...
- `Interest_Coverage` 是「流量/流量」比率（EBIT / 利息費用，兩者都是損益表當季數字），結構跟 [`margins/`](margins/) 一樣：只有單季跟 TTM 兩種口徑，沒有年化。
