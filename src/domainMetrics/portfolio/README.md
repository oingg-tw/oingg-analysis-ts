...
這是唯一 `scope` 不是 `Security` 的分類——這裡多數指標評估的是「一個投資組合（多檔標的的配置）」，不是單一公司。本服務目前的資料模型（`symbol` + `year` + `season`）只支援查詢單一公司單一季度，完全沒有「使用者持有哪些部位、權重多少」這種投資組合層級的資料結構。要做這一類，得先設計一套全新的組合資料模型（可能需要使用者自訂持股清單、權重、基準指數），跟現有「查某公司某季財報比率」是完全不同的產品形態，屬於架構層級的決定，不是加一個指標而已。

多數指標還需要**歷史報酬率序列**（每日/月報酬），這又回到缺市場價格時間序列的問題（`technicals` 分類 2026-09-05 已刪除，不再是可以類比的參照對象）。

**`Beta` 是這一類的例外**：雖然 taxonomy 把它跟其他投資組合指標放在同一類，但公式本身只需要「單一標的的日報酬率」跟「市場基準指數的日報酬率」，不需要使用者自訂的投資組合持股結構——本質上是 `scope: Security` 的指標，只是剛好被 taxonomy 歸進這個資料夾。2026-08-26 一開始用的是 oingg-mops-ts 的 `daily_market_index`（加權股價指數）跟 `daily_stock_price`（個股日成交），這兩張表 2026-08-30 從資料庫整個消失，改用 oingg-twse 的 `DailyTaiexIndex`（`daily_taiex_index`）跟 `DailyPrice`（`daily_price`），順帶讓覆蓋深度變得更好（6 家種子公司回填了約 5 年歷史）。其他指標（`Alpha`、`Sharpe_Ratio`⋯⋯）真正需要的「投資組合資料模型」問題還是沒解決，不要因為 Beta 做出來了就以為整類都解套了。
...
- **6 家種子公司歷史深度最完整**（2330/2881/2867/2801/2207/2855，2026-08-30 更新，改用 oingg-twse `daily_price` 後）：這 6 家回填了約 5 年歷史（2021-09 至今），三個窗口（1Y/2Y/5Y）都算得出來；其他公司多半只有近幾個月資料，長窗口可能因樣本數不足回傳 `calculation_error`，完全沒有資料的公司會在 `fieldStatuses` 標成 `not_applicable`（見下方「null 值規範」），不是伺服器錯誤；覆蓋率之後會持續成長，判斷邏輯是現查有沒有資料，不是寫死這份名單。
...
- `not_applicable`：這個指標對查詢對象結構性不適用（例如 `daily_price` 沒有這檔股票的資料，不是「還沒補齊」而是「目前完全沒有覆蓋」）。
