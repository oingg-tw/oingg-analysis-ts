...
// CAPM 無風險利率代理：10年期政府公債次級市場殖利率（月資料），來源見 prisma/cbc/schema.prisma
// 的 MonthlyGovBondYield10y 註解。asOfDate 當月尚未發布時（CBC 通常落後 1-2 個月），
...
export const getRiskFreeRateAsOf = async (asOfDate: Date): Promise<RiskFreeRateAsOf | null> => {
  ...
  const row = await cbcPrisma.monthlyGovBondYield10y.findFirst({
