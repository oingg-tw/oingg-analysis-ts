...
- **狀態**：⬜ 未實作，準備中。
- **跟 `../metrics/` 的關係**：`src/domainApi/metrics/` 底下的分類（`profitability`/`valuation`/`guru`⋯⋯）全部假設「一家公司 + 季度財報（MOPS）+ 股價（TWSE）」這個骨架，特別股不是新公司、也沒有自己的季度損益表可以拆解，硬塞進 `metrics/` 的心智模型會很彆扭，2026-08-31 使用者決定另外開一個平行分類。之後如果要做 REITs／ETF／興櫃／KY 股票專區，預期也會在這裡開平行的子資料夾，不是各自獨立的頂層分類。
