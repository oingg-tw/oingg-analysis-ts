...
//   （例如 model RoeResult -> metric key "roe"，跟資料夾 src/domainApi/metrics/profitability/roe/ 對應）。
...
export const checkFilterCatalogConsistency = (isProduction: boolean): void => {
  ...
  if (!isProduction) {
    throw new Error(
      `filterCatalog.ts 跟 prisma/analysis/schema.prisma 不一致（${problems.length} 個問題，詳見上方 log）。請同步更新 src/domainApi/filter/filterCatalog.ts 後再啟動。`,
    );
  }
};
