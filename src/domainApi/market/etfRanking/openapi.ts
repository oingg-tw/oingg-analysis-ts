...
export const registerEtfRankingOpenApi = (): void => {
  registry.registerPath({
    ...
    description:
      ...
      '國外成分證券/債券成分/槓桿型/反向型/多資產/連結式，主動式 ETF 沒有這個概念時是 null）。isActive（是否為主動式 ETF，不追蹤特定指數、' +
      '經理人自訂策略操作）是 sitca-ts 提供的權威欄位，不是從 category 猜的。belowStatutoryThreshold：規模是否低於法定下市門檻' +
      '（下市風險近似警示，sitca-ts 提供）。distributionFrequency 是從 distribution_class_info 拆出的配息頻率（月配/季配/半年配/年配/' +
      '一年兩次配息/其他/不分配），純客觀顯示欄位，不是投資建議。',
    tags: ['Market'],
    request: { query: getEtfRankingQuerySchema },
    responses: {
      200: { description: '前 limit 名的 ETF 排行。', content: { 'application/json': { schema: etfRankingResultSchema } } },
