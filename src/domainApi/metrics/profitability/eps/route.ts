 ...
 *
 *       跟季報自己申報的 eps/epsDiluted 不同——這裡是本服務用「淨利 / 股本歷史對應的流通股數」自己算出來的，
 *       口徑跟 ROE 一致（單季、單季年化、TTM 三種），可以互相對照驗證。
 *
 *       計算口徑：
 *       - 淨利欄位優先採用「歸屬於母公司」口徑（netIncomeAttributableToParent），缺漏時退回用整體數字（netIncome）。
 *       - epsQuarterly：本季淨利 / 本季報告日對應的流通股數。
 *       - epsQuarterlyAnnualized：epsQuarterly 簡單 x4，非以近四季實際加總計算。
 *       - epsTtm：近四季（含本季）淨利加總 / 本季報告日對應的流通股數，近四季資料須完整存在才會計算，否則為 null。
 ...
 */
registerCompanyRoute(router, '/eps', getEps);
