...
export const dividendHistoryEventSchema = z.object({
  ...
  cashDividendFromEarnings: z.number().meta({ description: '元／股，現金股利中「盈餘分配」的部分（可能含以前年度累積的盈餘，不一定是當年盈餘）' }),
  cashDividendFromLegalAndCapitalReserve: z.number().meta({ description: '元／股，現金股利中「法定盈餘公積、資本公積發放之現金」（MOPS 原始欄位，兩種來源合在一欄、無法拆開）' }),
...
export const dividendHistoryEntrySchema = z.object({
  ...
  cashDividendFromLegalAndCapitalReserve: z.number().meta({ description: '該年度各次「法定盈餘公積、資本公積發放之現金」加總，元／股' }),
  ...
  eps: z.number().nullable().meta({ description: '該年度年報公告的基本每股盈餘（eps.FY）；該年度沒有年報為 null' }),
  payoutRatio: z.number().nullable().meta({ description: '%，盈餘發放率 = cashDividendFromEarnings ÷ eps × 100——只算「盈餘分配之股東現金股利」，法定盈餘公積與資本公積發放不算；全部來自公積時為 0。EPS ≤ 0 或缺 EPS 時為 null（虧損年度不硬算）。2026-09-25 前分子是 cashDividend 合計' }),
