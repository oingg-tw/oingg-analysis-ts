...
// src/models/playwright/industryTree.ts 的 findPeerGroupByTree）的葉節點當第一選擇，
// 不再用 33 類 category 當第一選擇——樹狀結構（粗分類→產業→產業內區隔 1~3 層→公司）
// 比扁平兩層更貼近「真正共用上下游的同業」，尤其是「資訊設備」這種粗糙分類混雜筆電代工/
// 品牌/散熱件/工業電腦/伺服器等完全不同商業模式公司的問題，靠樹的細分區隔解決。
// category/coarseGroup/source 這組資訊仍然存在（company_category_summary 沒有下線），
// 但降級為純資訊性欄位（這家公司的「產業標籤」），不再是同業比較演算法本身的一部分。
...
export const getCompanyPeerGroup = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const result = findPeerGroupByTree(symbol, candidatePool, minPeers);
    const categoryInfo = getCompanyCategoryInfo(symbol);
