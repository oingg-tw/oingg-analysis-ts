...
// - identity：跟這檔 ETF「是什麼」有關，不是數字績效
// - sizeAndFlow：規模/資金流向
// - navAndPrice：淨值/市價（折溢價）
// - performance：報酬率
// - cost：總費用率（含逐年序列）+ 費用細項拆分
export type EtfCategoryKey = 'identity' | 'sizeAndFlow' | 'navAndPrice' | 'performance' | 'cost';
...
export const NUMERIC_FIELDS: Record<string, NumericFieldDefinition> = {
  aum: { kind: 'numeric', field: 'aum', label: '規模', unit: '元', categoryKey: 'sizeAndFlow', sqlColumn: 'aum' },
  holders: { kind: 'numeric', field: 'holders', label: '受益人數', unit: '人', categoryKey: 'sizeAndFlow', sqlColumn: 'holders' },
  netFlow: { kind: 'numeric', field: 'netFlow', label: '淨申購（申購-贖回）', unit: '元', categoryKey: 'sizeAndFlow', sqlColumn: 'net_flow' },
  dcaAmount: { kind: 'numeric', field: 'dcaAmount', label: '定期定額金額', unit: '元', categoryKey: 'sizeAndFlow', sqlColumn: 'dca_amount' },
  marketShareRate: { kind: 'numeric', field: 'marketShareRate', label: '市占率', unit: '%', categoryKey: 'sizeAndFlow', sqlColumn: 'market_share_rate' },
  nav: { kind: 'numeric', field: 'nav', label: '淨值', unit: '元', categoryKey: 'navAndPrice', sqlColumn: 'nav' },
  return3m: { kind: 'numeric', field: 'return3m', label: '近3月報酬率', unit: '%', categoryKey: 'performance', sqlColumn: 'return_3m' },
  return6m: { kind: 'numeric', field: 'return6m', label: '近6月報酬率', unit: '%', categoryKey: 'performance', sqlColumn: 'return_6m' },
  return1y: { kind: 'numeric', field: 'return1y', label: '近1年報酬率', unit: '%', categoryKey: 'performance', sqlColumn: 'return_1y' },
  return2y: { kind: 'numeric', field: 'return2y', label: '近2年報酬率', unit: '%', categoryKey: 'performance', sqlColumn: 'return_2y' },
  return3y: { kind: 'numeric', field: 'return3y', label: '近3年報酬率', unit: '%', categoryKey: 'performance', sqlColumn: 'return_3y' },
  return5y: { kind: 'numeric', field: 'return5y', label: '近5年報酬率', unit: '%', categoryKey: 'performance', sqlColumn: 'return_5y' },
  returnYtd: { kind: 'numeric', field: 'returnYtd', label: '今年以來報酬率', unit: '%', categoryKey: 'performance', sqlColumn: 'return_ytd' },
  return10y: { kind: 'numeric', field: 'return10y', label: '近10年報酬率', unit: '%', categoryKey: 'performance', sqlColumn: 'return_10y' },
  // 只用「最新一個完整年度」，發行日在那個基準年（或更晚）的 ETF 沒有完整年度可比，值是
  // null——跟 etfRanking 的 expenseRatio 同一套規則，見 queryBuilder.ts 的說明。
  expenseRatio: { kind: 'numeric', field: 'expenseRatio', label: '總費用率', unit: '%', categoryKey: 'cost', sqlColumn: 'expense_ratio', needsExpenseJoin: true },
  // 2026-09-04 sitca-ts 新增欄位——法定下市規模門檻，純資訊性數字，跟 belowStatutoryThreshold
  // （下面 CATEGORICAL_FIELDS）是同一組資料的一體兩面：這是門檻本身，那個是「是否低於門檻」。
  statutoryAumThreshold: {
    kind: 'numeric',
    field: 'statutoryAumThreshold',
    label: '法定下市規模門檻',
    unit: '元',
    categoryKey: 'sizeAndFlow',
    sqlColumn: 'statutory_aum_threshold',
  },
  ...
  premiumDiscountPct: {
    ...
    label: '折溢價率',
    unit: '%',
    categoryKey: 'navAndPrice',
    sqlColumn: 'premium_discount_pct',
    needsPremiumDiscountJoin: true,
  },
};
...
export const DATE_FIELDS: Record<string, DateFieldDefinition> = {
  ...
  establishedDate: { kind: 'date', field: 'establishedDate', label: '成立日', categoryKey: 'identity', sqlColumn: 'established_date' },
};
...
export const NUMERIC_FIELDS_FEE_BREAKDOWN: Record<string, NumericFieldDefinition> = {
  managementFeeRate: { kind: 'numeric', field: 'managementFeeRate', label: '經理費率（最新完整年度）', unit: '%', categoryKey: 'cost', sqlColumn: 'management_fee_rate', needsExpenseLatestFullYearJoin: true },
  custodianFeeRate: { kind: 'numeric', field: 'custodianFeeRate', label: '保管費率（最新完整年度）', unit: '%', categoryKey: 'cost', sqlColumn: 'custodian_fee_rate', needsExpenseLatestFullYearJoin: true },
  guaranteeFeeRate: { kind: 'numeric', field: 'guaranteeFeeRate', label: '保證費率（最新完整年度）', unit: '%', categoryKey: 'cost', sqlColumn: 'guarantee_fee_rate', needsExpenseLatestFullYearJoin: true },
  otherFeeRate: { kind: 'numeric', field: 'otherFeeRate', label: '其他費用率（最新完整年度）', unit: '%', categoryKey: 'cost', sqlColumn: 'other_fee_rate', needsExpenseLatestFullYearJoin: true },
  commissionRate: { kind: 'numeric', field: 'commissionRate', label: '手續費率（最新完整年度）', unit: '%', categoryKey: 'cost', sqlColumn: 'commission_rate', needsExpenseLatestFullYearJoin: true },
  transactionTaxRate: { kind: 'numeric', field: 'transactionTaxRate', label: '交易稅率（最新完整年度）', unit: '%', categoryKey: 'cost', sqlColumn: 'transaction_tax_rate', needsExpenseLatestFullYearJoin: true },
  etfTradingFeeRate: { kind: 'numeric', field: 'etfTradingFeeRate', label: 'ETF買賣手續費率（最新完整年度）', unit: '%', categoryKey: 'cost', sqlColumn: 'etf_trading_fee_rate', needsExpenseLatestFullYearJoin: true },
};
...
export const CATEGORICAL_FIELDS: Record<string, CategoricalFieldDefinition> = {
  ...
  isActive: { kind: 'categorical', field: 'isActive', label: '主動式ETF', categoryKey: 'identity', sqlColumn: 'is_active', staticValues: ['true', 'false'], isBoolean: true },
  // 2026-09-04 sitca-ts 新增欄位——下市風險近似警示（規模低於法定門檻），見 queryBuilder.ts
  // 的說明，跟 NUMERIC_FIELDS.statutoryAumThreshold 是同一組資料的一體兩面。歸類在
  // sizeAndFlow（跟門檻本身同一組），不是 identity——這是規模相關的風險警示，不是身分分類。
  belowStatutoryThreshold: {
    kind: 'categorical',
    field: 'belowStatutoryThreshold',
    label: '規模低於法定下市門檻',
    categoryKey: 'sizeAndFlow',
    sqlColumn: 'below_statutory_threshold',
  ...
  distributionFrequency: { kind: 'categorical', field: 'distributionFrequency', label: '配息頻率', categoryKey: 'identity', sqlColumn: 'distribution_frequency' },
};
...
// 2026-09-11 使用者要求比照股票 GET /filters 的巢狀分類，順序即前端選單顯示順序。
export const ETF_CATEGORIES: { key: EtfCategoryKey; displayName: string }[] = [
  { key: 'identity', displayName: '基本資料' },
  { key: 'sizeAndFlow', displayName: '規模與資金' },
  { key: 'navAndPrice', displayName: '淨值與市價' },
  { key: 'performance', displayName: '績效表現' },
  { key: 'cost', displayName: '成本費用' },
];
