...
import { periodTypeGroup, rollingWindowGroup, snapshotCadenceGroup } from '@/application/metrics/metricValueWriter';
import type { PeriodType, LookbackRange, SamplingInterval, SnapshotCadence } from '@/domain/metrics/metricBasis';
...
// 落在其中一組，這裡依 metricCode 自動判斷該用哪一組的解析規則，timeframe 格式錯誤或不在
...
// metricCode 實際可用的 timeframe 清單」，呼叫端直接拿來當選單，不用自己組合。2026-09-09
...
// 端點的 query 已經有獨立的 metricCode 參數，不需要再解析 "metricCode.timeframe" 這種複合字串，
// 只需要「給定 metricCode，這個 timeframe 屬於四組裡的哪一組、合不合法」這一段判斷邏輯，
...
export const resolveTimeframeForMetric = (metricCode: string, timeframe: string, displayField: string): FieldRef => {
  ...
  switch (definition.group) {
    case 'period': {
      if (!definition.allowedPeriodTypes.includes(timeframe as PeriodType)) {
        throw new ScreenerValidationError(`"${displayField}" 不是可查詢的欄位——metricCode "${metricCode}" 不支援 periodType "${timeframe}"，允許的值：${definition.allowedPeriodTypes.join(', ')}。`);
      }
    ...
    case 'rollingWindow': {
      ...
      // 通過，查詢卻永遠是空結果）——改成看 allowedRollingWindowTimeframes（唯一正確的合法
      // 組合清單），不是各自檢查兩個獨立陣列的 includes()。
      if (!definition.allowedRollingWindowTimeframes.includes(timeframe)) {
        throw new ScreenerValidationError(
          `"${displayField}" 不是可查詢的欄位——metricCode "${metricCode}" 的 timeframe 要是 "<lookbackRange>_<samplingInterval>" 格式，且必須是下列已知有資料的組合之一（不是 lookbackRange/samplingInterval 的自由交叉組合）：${definition.allowedRollingWindowTimeframes.join(', ')}。`,
        );
      }
    ...
    case 'snapshot': {
      if (!definition.allowedSnapshotCadences.includes(timeframe as SnapshotCadence)) {
        throw new ScreenerValidationError(`"${displayField}" 不是可查詢的欄位——metricCode "${metricCode}" 不支援 snapshotCadence "${timeframe}"，允許的值：${definition.allowedSnapshotCadences.join(', ')}。`);
      }
...
export const resolveFieldOrThrow = (field: string): FieldRef => {
  ...
  if (firstDot === -1) {
    throw new ScreenerValidationError(`"${field}" 格式錯誤，field 要是 "metricCode.timeframe" 這種格式（例如 "roe.TTM"），可用的 metricCode/timeframe 組合見 GET /metrics。`);
  }
