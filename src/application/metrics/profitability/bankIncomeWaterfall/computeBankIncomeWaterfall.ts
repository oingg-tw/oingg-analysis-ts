...
// 非銀行 / 查無任何一季時 8 個 slot 全部同一種 skip（型別別名在下面才宣告，型別層級可以先引用）。
const skippedAll = (action: 'skipped_no_quarter' | 'skipped_no_knowledge_date'): BankIncomeWaterfallComputationBatch['slots'] => ({
...
export type BankIncomeWaterfallDeps = Pick<PitDeps, 'statements' | 'quarters' | 'announcements' | 'shares' | 'industry'>;
...
export const computeBankIncomeWaterfall = async (query: QuarterlyMetricQuery, deps: BankIncomeWaterfallDeps): Promise<BankIncomeWaterfallComputationBatch> => {
  ...
  const statement = await deps.statements.getBankIncomeStatement(key);
  ...
  const ttmRecords = await Promise.all(
    ttmQuarters.map((tq) => deps.statements.getBankIncomeStatement({ symbol, year: Number(tq.year), quarter: Number(tq.season), dataType, subsidiaryCompanyId }))
  );
  ...
  let bankBadDebtProvisionPerShareTtm: ComputationSlot;
  let bankOtherOperatingExpensePerShareTtm: ComputationSlot;
  ...
  if (ttmComplete) {
    ...
    if (!ttmAnchor) {
      ...
      bankNetInterestIncomePerShareTtm = computation({ ...coordinateFor('bankNetInterestIncomePerShare'), ...periodTypeGroup('TTM'), value: netInterestTtmCalc.value, nullReason: netInterestTtmCalc.nullReason, knowledgeDate, knowledgeDateIsFallback });
      bankNetNonInterestIncomePerShareTtm = computation({ ...coordinateFor('bankNetNonInterestIncomePerShare'), ...periodTypeGroup('TTM'), value: netNonInterestTtmCalc.value, nullReason: netNonInterestTtmCalc.nullReason, knowledgeDate, knowledgeDateIsFallback });
      bankBadDebtProvisionPerShareTtm = computation({ ...coordinateFor('bankBadDebtProvisionPerShare'), ...periodTypeGroup('TTM'), value: badDebtTtmCalc.value, nullReason: badDebtTtmCalc.nullReason, knowledgeDate, knowledgeDateIsFallback });
      bankOtherOperatingExpensePerShareTtm = computation({ ...coordinateFor('bankOtherOperatingExpensePerShare'), ...periodTypeGroup('TTM'), value: otherOpexTtmCalc.value, nullReason: otherOpexTtmCalc.nullReason, knowledgeDate, knowledgeDateIsFallback });
    }
  } else if (mainAnchor) {
    const { knowledgeDate, isFallback: knowledgeDateIsFallback } = mainAnchor;
    bankNetInterestIncomePerShareTtm = computation({ ...coordinateFor('bankNetInterestIncomePerShare'), ...periodTypeGroup('TTM'), value: null, nullReason: 'insufficient_history', knowledgeDate, knowledgeDateIsFallback });
    bankNetNonInterestIncomePerShareTtm = computation({ ...coordinateFor('bankNetNonInterestIncomePerShare'), ...periodTypeGroup('TTM'), value: null, nullReason: 'insufficient_history', knowledgeDate, knowledgeDateIsFallback });
    bankBadDebtProvisionPerShareTtm = computation({ ...coordinateFor('bankBadDebtProvisionPerShare'), ...periodTypeGroup('TTM'), value: null, nullReason: 'insufficient_history', knowledgeDate, knowledgeDateIsFallback });
    bankOtherOperatingExpensePerShareTtm = computation({ ...coordinateFor('bankOtherOperatingExpensePerShare'), ...periodTypeGroup('TTM'), value: null, nullReason: 'insufficient_history', knowledgeDate, knowledgeDateIsFallback });
  } else {
    bankNetInterestIncomePerShareTtm = { action: 'skipped_no_knowledge_date' };
    bankNetNonInterestIncomePerShareTtm = { action: 'skipped_no_knowledge_date' };
    bankBadDebtProvisionPerShareTtm = { action: 'skipped_no_knowledge_date' };
