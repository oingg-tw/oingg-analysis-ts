...
export type BankAssetQualityFamilyDeps = Pick<PitDeps, 'statements' | 'quarters' | 'announcements' | 'industry'>;
...
export const computeBankAssetQualityFamily = async (query: QuarterlyMetricQuery, deps: BankAssetQualityFamilyDeps): Promise<BankAssetQualityFamilyComputationBatch> => {
  ...
  if (!mainAnchor) {
    ...
    bankNplCoverageRatio = computation({
      ...coordinateBase,
  ...
  return { symbol, rocYear: String(rocYear), season: String(seasonNum), slots: { bankNplRatio, bankNplCoverageRatio } };
};
