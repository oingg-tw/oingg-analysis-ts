...
// 2026-09-22 公式照 Chan, Jegadeesh & Lakonishok（1996）版（見 computeSue.ts）：σ 是前 8 季盈餘變動值的
// 統計估計，不是可逐格核對的原始事實，所以 entries 只列「構成本季盈餘變動值」的 2 筆（本季/去年同季
// 淨利），沒有股數（用淨利金額不用 EPS），8 季樣本本身用 methodologyNote 講清楚。
...
export const getSueProvenance = async (query: QuarterlyMetricQuery, deps: SueDeps): Promise<MetricProvenanceResult> => {
  ...
  return {
    ...
    methodologyNote:
      'Chan, Jegadeesh & Lakonishok（1996）定義：SUE =（本季淨利 − 去年同季淨利）/ σ，σ 是前 8 季「單季淨利 − 去年同季淨利」的樣本標準差；這裡只列出構成本季變動值的 2 期原始欄位，8 季樣本本身不逐筆列出。',
  };
};
