...
// 沒有季度座標（見上方說明），batch 也就沒有 rocYear/season——不用通用的 ComputationBatch，
// 攤平後的舊 outcome 形狀是 { symbol, ttm }。
export interface DividendDistributionCountComputationBatch {
  symbol: string;
  slots: Record<'ttm', ComputationSlot>;
}
...
export const computeDividendDistributionCount = async (query: QuarterlyMetricQuery, deps: DividendDistributionCountDeps): Promise<DividendDistributionCountComputationBatch> => {
  const { symbol, dataType, subsidiaryCompanyId } = query;
  const events = await deps.dividendEvents.getDividendDistributionEvents(symbol);

  if (events.length === 0) {
    return { symbol, slots: { ttm: { action: 'skipped_no_quarter' } } };
  }
