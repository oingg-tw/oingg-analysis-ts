...
export type MarketRatiosDeps = Pick<PitDeps, 'market'>;

export type MarketRatiosComputationBatch = DailyComputationBatch<'exchangePeRatio' | 'exchangePbRatio' | 'dividendYield'>;

export const computeMarketRatios = async (query: MarketRatiosPitQuery, deps: MarketRatiosDeps): Promise<MarketRatiosComputationBatch> => {
  const { symbol, date, dataType, subsidiaryCompanyId } = query;

  const valuation = await deps.market.getDailyValuation(symbol, date);
  if (!valuation) {
    return { symbol, tradeDate: null, slots: { exchangePeRatio: { action: 'skipped_no_trade_date' }, exchangePbRatio: { action: 'skipped_no_trade_date' }, dividendYield: { action: 'skipped_no_trade_date' } } };
  }
  ...
  const exchangePeRatio = computation({
  ...
  const exchangePbRatio = computation({
  ...
  const dividendYield = computation({
  ...
  return { symbol, tradeDate: tradeDate.toISOString().slice(0, 10), slots: { exchangePeRatio, exchangePbRatio, dividendYield } };
};
