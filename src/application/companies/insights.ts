...
export const getCompanyMetricCompleteness = async (symbol: string, deps: Pick<AppDeps, 'metricValueQueries'>): Promise<CompanyMetricCompletenessResult> => {
...
export const getCompanyPiotroskiBreakdown = ({ symbol, year, season }: QuarterlyLookupQuery, deps: PiotroskiFScoreDeps): Promise<PiotroskiFScoreBreakdown> =>
  getPiotroskiFScoreBreakdown({ symbol, year, season, dataType: '2', subsidiaryCompanyId: '' }, deps);
...
export const getCompanyMetricProvenance = (symbol: string, { metricCode, year, season }: GetCompanyMetricProvenanceQuery, deps: PitDeps): Promise<MetricProvenanceResult> =>
  createProvenanceResolvers(deps)[metricCode]({ symbol, year, season, dataType: '2', subsidiaryCompanyId: '' });
