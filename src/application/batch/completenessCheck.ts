...
// 指標的 calculate* 函式，upsert 那段本來就自己包 try/catch 吞掉
...
// 的拆表決策）。目前 dailyIndicatorJobs（./daily/indicatorRegistry.ts）是
...
// 2026-09-17 Phase 4：從 http/batch/completenessCheck.ts 搬到 application，寫入列數查詢改走
// deps.metricValueQueries（MetricValueQueryPort.countMetricRowsWrittenSince）、logger 由 deps 注入。
...
export type CompletenessCheckDeps = Pick<AppDeps, 'logger' | 'metricValueQueries'>;
...
export const checkJobCompleteness = async (job: IndicatorJob, companyIds: string[], batchStartedAt: Date, deps: CompletenessCheckDeps): Promise<CompletenessResult> => {
