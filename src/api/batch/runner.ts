// 批次入口的實際執行邏輯——跟 API 入口（src/api/bff/**/{controller,route}.ts）平行的另一個
...
// 1. HTTP：src/api/batch/route.ts 的 `POST /batch/compute`，給 GCP Cloud Scheduler 直接
//    打，跟 api/bff 共用同一個 Cloud Run Service（長駐行程）。
...
// src/api/batch/indicatorRegistry.ts。
//
...
// api/batch/route.ts 也會在跟 api/bff 共用的同一個長駐伺服器行程裡呼叫這支函式，
