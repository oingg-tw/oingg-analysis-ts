...
export const registerQuarterlyBatchOpenApi = (): void => {
  registry.registerPath({
    ...
    description:
      '涵蓋 profitability（11 支）、cashFlow（4 支）、resilience（5 支）、turnover（2 支）、guru（9 支）、' +
      'valuation 的 psr/pFcf/evEbitda（3 支）——這些都依賴 mops 季度財報，一家公司一季頂多變一次，跟 daily ' +
      '那組（依賴每日股價/市場行情）刻意分開，避免每天對財報資料白算一次。' +
      '跟 api/bff 的各支指標端點共用同一份計算邏輯（見 src/domainMetrics/），差別是這支端點一次跑全市場每家公司，' +
      '不是單一公司查詢。實際邏輯在 src/api/batch/runner.ts 的 runBatchCompute，跑完才回應（同步），視資料量可能要好幾分鐘，' +
