...
import { z } from 'zod';
import { calculateDupont } from './service';
...
export const getDupont = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const result = await calculateDupont(validationResult.data);
    res.status(200).json(result);
  } catch (error) {
    console.error('DuPont analysis calculation failed:', error);
    next(error);
  }
};
