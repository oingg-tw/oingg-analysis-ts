...
import { calculateNetDebtToEbitda } from '@/domains/solvency/netDebtToEbitda/service';
...
const resolveQuarter = async (
  ...
  return getLatestAvailableQuarter(companyId, dataType, subsidiaryCompanyId, ['balanceSheet', 'incomeStatement', 'cashFlowStatement']);
};
...
export const calculateEvEbitda = async (query: EvEbitdaQuery): Promise<EvEbitdaResult> => {
  ...
  if (priceAnchor) {
    ...
    if (marketCap) {
      ...
      warnings.push(`查無 ${companyId} 在 ${priceAnchor.date.toISOString().slice(0, 10)} 或之前的股價/股本資料，企業價值無法計算，見 fieldStatuses。`);
    }
  }
