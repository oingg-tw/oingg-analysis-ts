...
import { z } from 'zod';
import { calculateEvEbitda } from './service';
...
export const getEvEbitda = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const result = await calculateEvEbitda(validationResult.data);
    res.status(200).json(result);
  } catch (error) {
    console.error('EV_EBITDA calculation failed:', error);
    next(error);
  }
};
