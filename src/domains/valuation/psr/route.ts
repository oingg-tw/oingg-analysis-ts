 ...
 * /valuation/psr:
 *   get:
 *     summary: 計算單一公司股價營收比（PSR）
 *     description: >
 *       直接讀取 oingg-mops-ts 已寫入資料庫的損益表資料，加上 oingg-twse 的 `daily_price`（股價）
 *       計算，本服務本身不向任何來源抓取資料。
 *
 *       計算口徑：
 *       - `PSR = 市值 / 營收`。市值是存量、營收是流量，拿市值除以「一季」的營收沒有標準意義，
 *         所以只提供 `psrQuarterlyAnnualized`（本季營收簡單 x4）跟 `psrTtm`（近四季實際加總）
 *         兩種口徑，沒有純單季版本——跟 [`../../solvency/netDebtToEbitda/`](../../solvency/netDebtToEbitda/route.ts) 同一種道理。
 *       - 營收直接引用 [`revenuePerShare`](../../profitability/revenuePerShare/route.ts) 已經算好的
 *         單季/TTM 數值，不重複查詢。
