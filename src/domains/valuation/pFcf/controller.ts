...
export const getPFcf = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const result = await calculatePFcf(validationResult.data);
    res.status(200).json(result);
  } catch (error) {
    console.error('P_FCF calculation failed:', error);
    next(error);
  }
};
