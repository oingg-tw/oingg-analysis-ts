...
import { getPFcf } from './controller';
 ...
 * /valuation/p-fcf:
 *   get:
 *     summary: 計算單一公司股價自由現金流比（P_FCF）
 *     description: >
 *       直接讀取 oingg-mops-ts 已寫入資料庫的現金流量表資料，加上 oingg-twse 的 `daily_price`
 *       （股價）計算，本服務本身不向任何來源抓取資料。
 *
 *       計算口徑：
 *       - `P_FCF = 市值 / 自由現金流（FCF）`；`FCF = 營業活動現金流量 + 資本支出`（`capitalExpenditures`
 *         在資料庫裡本來就是負數，所以是加不是減）。市值是存量、FCF 是流量，拿市值除以「一季」的
 *         FCF 沒有標準意義，所以只提供 `pFcfQuarterlyAnnualized`（本季 FCF 簡單 x4）跟 `pFcfTtm`
 *         （近四季實際加總）兩種口徑，沒有純單季版本——跟 [`../psr/`](../psr/route.ts)、
 *         [`../../solvency/netDebtToEbitda/`](../../solvency/netDebtToEbitda/route.ts) 同一種道理。
 *       - FCF 直接引用 [`cashFlowPerShare`](../../cashFlow/cashFlowPerShare/route.ts) 已經算好的
 *         單季/TTM 營業活動現金流量、資本支出，不重複查詢。
 ...
 *         `warnings` 註明可能有 look-ahead bias，見 [`../../../shared/reportAnnouncementDate.ts`](../../../shared/reportAnnouncementDate.ts)。
