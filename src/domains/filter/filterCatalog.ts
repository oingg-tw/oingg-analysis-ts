...
// key 對應 src/domains/metrics 底下的資料夾/檔案結構（2026-08-31 從 src/domains 底下搬進 metrics/
// 子資料夾，跟同層新開的 src/domains/preferredStock/ 分開——後者不是「公司季度財報」骨架的指標，
...
export interface FilterMetric {
  /** 對應 src/domains/metrics/<category>/<key> 資料夾名稱；同一個底層 API 拆成多個顯示分組時
...
export interface FilterCategory {
  /** 對應 src/domains/metrics/<key> 資料夾名稱 */
  key: string;
  /** 直接顯示給前端使用者看的文案——不放實作細節/內部路徑/標點符號/period 資訊，見檔案開頭說明 */
  name: string;
  ...
  },
  {
    ...
    metrics: [
      ...
      },
      {
        ...
        // src/domains/metrics/guru/README.md，公式細節屬於文件該講的事，不放進 name。
        ...
        fields: [
          ...
          { key: 'tlta', name: 'TLTA', period: 'snapshot', sort: 4, description: '總負債÷總資產，財務槓桿程度' },
          {
            key: 'wcta',
            name: 'WCTA',
            period: 'snapshot',
          ...
          { key: 'oeneg', name: 'OENEG', period: 'snapshot', sort: 7, description: '總負債是否大於總資產的二元訊號（1 代表是，資不抵債；0 代表否）' },
          { key: 'nita', name: 'NITA', period: 'ttm', sort: 8, description: '稅後淨利（TTM）÷總資產，資產報酬率，概念跟 ROA 相同' },
          {
            key: 'futl',
            name: 'FUTL',
            period: 'ttm',
          ...
          { key: 'intwo', name: 'INTWO', period: 'snapshot', sort: 10, description: '近兩年 TTM 稅後淨利是否都是負值的二元訊號（1 代表連續兩年虧損）' },
          {
            key: 'chin',
            name: 'CHIN',
            period: 'snapshot',
