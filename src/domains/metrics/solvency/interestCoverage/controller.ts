...
export const getInterestCoverage = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const result = await calculateInterestCoverage(validationResult.data);
    await sendWithCompanyName(res, result);
  } catch (error) {
    console.error('Interest coverage calculation failed:', error);
    next(error);
  }
};
