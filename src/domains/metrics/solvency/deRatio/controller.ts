...
import { z } from 'zod';
import { calculateDeRatio } from './service';
...
export const getDeRatio = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const result = await calculateDeRatio(validationResult.data);
    await sendWithCompanyName(res, result);
  } catch (error) {
    console.error('DE ratio calculation failed:', error);
    next(error);
  }
};
