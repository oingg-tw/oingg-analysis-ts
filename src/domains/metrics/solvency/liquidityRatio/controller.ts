...
export const getLiquidityRatio = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const result = await calculateLiquidityRatio(validationResult.data);
    await sendWithCompanyName(res, result);
  } catch (error) {
    console.error('Liquidity ratio calculation failed:', error);
    next(error);
  }
};
