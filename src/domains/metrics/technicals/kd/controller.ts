...
import { z } from 'zod';
import { calculateKd } from './service';
...
export const getKd = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const result = await calculateKd(validationResult.data);
    await sendWithCompanyName(res, result);
  } catch (error) {
    console.error('KD calculation failed:', error);
    next(error);
  }
};
