...
import { z } from 'zod';
import { calculateRoce } from './service';
...
export const getRoce = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const result = await calculateRoce(validationResult.data);
    res.status(200).json(result);
  } catch (error) {
    console.error('ROCE calculation failed:', error);
    next(error);
  }
};
