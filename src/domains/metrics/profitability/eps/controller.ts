...
export const getEps = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const result = await calculateEps(validationResult.data);
    await sendWithCompanyName(res, result);
  } catch (error) {
    console.error('EPS calculation failed:', error);
    next(error);
  }
};
