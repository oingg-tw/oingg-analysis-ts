...
import { z } from 'zod';
import { calculateSgr } from './service';
...
export const getSgr = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const result = await calculateSgr(validationResult.data);
    res.status(200).json(result);
  } catch (error) {
    console.error('SGR calculation failed:', error);
    next(error);
  }
};
