...
export const getBvps = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const result = await calculateBvps(validationResult.data);
    await sendWithCompanyName(res, result);
  } catch (error) {
    console.error('BVPS calculation failed:', error);
    next(error);
  }
};
