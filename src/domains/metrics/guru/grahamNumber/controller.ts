...
export const getGrahamNumber = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const result = await calculateGrahamNumber(validationResult.data);
    res.status(200).json(result);
  } catch (error) {
    console.error('Graham Number calculation failed:', error);
    next(error);
  }
};
