...
export const getAltmanZScore = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const result = await calculateAltmanZScore(validationResult.data);
    await sendWithCompanyName(res, result);
  } catch (error) {
    console.error('Altman Z-Score calculation failed:', error);
    next(error);
  }
};
