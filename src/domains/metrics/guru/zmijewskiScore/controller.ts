...
export const getZmijewskiScore = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const result = await calculateZmijewskiScore(validationResult.data);
    res.status(200).json(result);
  } catch (error) {
    console.error('Zmijewski Score calculation failed:', error);
    next(error);
  }
};
