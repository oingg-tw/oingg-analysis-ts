...
export const getPsr = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const result = await calculatePsr(validationResult.data);
    res.status(200).json(result);
  } catch (error) {
    console.error('PSR calculation failed:', error);
    next(error);
  }
};
