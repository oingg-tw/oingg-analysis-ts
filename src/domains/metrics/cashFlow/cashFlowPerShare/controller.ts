...
export const getCashFlowPerShare = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const result = await calculateCashFlowPerShare(validationResult.data);
    await sendWithCompanyName(res, result);
  } catch (error) {
    console.error('Cash flow per share calculation failed:', error);
    next(error);
  }
};
