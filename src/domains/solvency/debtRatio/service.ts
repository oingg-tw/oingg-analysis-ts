...
const emptyResult = (companyId: string, dataType: '1' | '2', subsidiaryCompanyId: string, warnings: string[]): DebtRatioResult => ({
  companyId,
  year: null,
  season: null,
  dataType,
...
export const calculateDebtRatio = async (query: DebtRatioQuery): Promise<DebtRatioResult> => {
  ...
  if (!resolvedQuarter) {
    return emptyResult(companyId, dataType, subsidiaryCompanyId, ['查無任何一季資產負債表有資料的季度，無法決定要用哪一季計算負債比率。']);
  }
