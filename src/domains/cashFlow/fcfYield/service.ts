...
import { getStockPriceAsOf, hasStockPriceCoverage } from '@/shared/marketCap';
...
export const calculateFcfYield = async (query: FcfYieldQuery): Promise<FcfYieldResult> => {
  ...
  if (priceAnchor) {
    ...
    const stockPrice = await getStockPriceAsOf(companyId, priceAnchor.date);
    if (stockPrice) {
      stockPriceValue = stockPrice.closePrice;
      stockPriceTradeDate = stockPrice.tradeDate;
    } else {
      warnings.push(`查無 ${companyId} 在 ${priceAnchor.date.toISOString().slice(0, 10)} 或之前的股價資料，FCF_Yield 無法計算，見 fieldStatuses。`);
    }
  }
  ...
  const stockPriceCovered = stockPriceValue === null ? await hasStockPriceCoverage(companyId) : true;
