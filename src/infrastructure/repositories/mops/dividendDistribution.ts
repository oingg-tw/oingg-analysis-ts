...
export const listDividendDistributionRows = async (symbol: string): Promise<DividendDistributionRow[]> => {
  ...
  return rows.map((r) => ({
    ...
    cashDividendFromLegalAndCapitalReserve: toNumberOrNull(r.cash_dividend_from_capital_reserve),
    stockDividendFromEarnings: toNumberOrNull(r.stock_dividend_from_earnings),
    stockDividendFromLegalAndCapitalReserve: toNumberOrNull(r.stock_dividend_from_capital_reserve),
