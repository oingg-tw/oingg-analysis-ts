...
export const dividendPayoutRatioDefinition: MetricDefinitionSpec = {
  ...
  tier: 'derived',
  // 2026-09-10 更正：badge.threshold 原本寫「< 60%」是誤植（web-nuxt 轉來的原始資料也是
  // 這個版本），實際去讀 Fidelity 這份文件的原文（Fruhan/Morrow/Hebard/Rahman, "Payout
  // Ratio: The Most Influential Management Decision a Company Can Make?", Jan 2013）
  // 確認過：文件的結論是「最適發放率區間是 40%–60%」，不是單邊「低於 60% 就安全」——
  // 論文明確指出過低（<40%）代表公司沒有善用資本配置優勢，過高（尤其 >80%）則會被市場
  // 質疑股利可持續性、推升資金成本，是一個雙邊區間論點，不是單邊安全上限。改用新增的
  // 'in_range' comparator（valueMin/valueMax）正確表達，不能硬套 lt/60 這種單邊比較。
  academicSourceUrl:
