...
// GET /metrics 全部指標（不限有 badge 的），各自查一筆代表性 token 的最新值，回傳
// hasValue/nullReason，讓「這家公司哪些指標實際有算出數字、哪些卡在什麼原因」一次看完。
//
// 代表性 token 的選法：優先選 TTM（季報型指標最常見、最少受單季雜訊干擾的口徑），
// 其次退回 validTokens 的第一個——同一 metricCode 底下不同 token 通常是同一份底層資料的
// 不同聚合方式，會不會有值高度相關，不需要對每個 metricCode 掃全部 token 組合（那是
// GET /companies/metric-history 自己的事，這裡只要「有沒有算出來過」這個訊號）。
const pickRepresentativeToken = (validTokens: string[]): string | undefined => (validTokens.includes('TTM') ? 'TTM' : validTokens[0]);

export interface CompanyMetricCompletenessEntry {
  metricCode: string;
  name: string;
  nameEn?: string;
  token: string | null; // null 代表這支 metricCode 沒有任何可用 token（目前沒有已知案例，防呆）
  hasValue: boolean;
    ...
    catalog.map(async ({ categoryKey, categoryDisplayName, metrics }) => {
      const entries = await Promise.all(
        metrics.map(async (metric): Promise<CompanyMetricCompletenessEntry> => {
          const token = pickRepresentativeToken(metric.validTokens);
          if (!token) {
            return { metricCode: metric.metricCode, name: metric.name, nameEn: metric.nameEn, token: null, hasValue: false, nullReason: null };
          }
