...
export const computeAndWriteCashFlowValuationFamilyPit = async (
  ...
  if (!ttmComplete) {
    ...
    const insufficientHistory = async (metricCode: string): Promise<BasisOutcome> =>
      writeMetricValue({ ...coordinateFor(metricCode), ...periodTypeGroup('TTM'), value: null, nullReason: 'insufficient_history', knowledgeDate, knowledgeDateIsFallback });

    return {
      symbol,
      rocYear: year,
      season,
      evToOcf: await insufficientHistory('evToOcf'),
      evToSales: await insufficientHistory('evToSales'),
      priceToOcf: await insufficientHistory('priceToOcf'),
      debtToFcf: await insufficientHistory('debtToFcf'),
      capexToOcfRatio: await insufficientHistory('capexToOcfRatio'),
      croic: await insufficientHistory('croic'),
      ocfMargin: await insufficientHistory('ocfMargin'),
      fcfConversionRate: await insufficientHistory('fcfConversionRate'),
    };
  }
  ...
  const write = (metricCode: string, value: number | null, nullReason: MetricNullReason | null): Promise<BasisOutcome> =>
    writeMetricValue({ ...coordinateFor(metricCode), ...periodTypeGroup('TTM'), value, nullReason, knowledgeDate, knowledgeDateIsFallback });

  const evToOcf = await write('evToOcf', evToOcfValue, nullReasonFor(evToOcfValue, enterpriseValue !== null));
  const evToSales = await write('evToSales', evToSalesValue, nullReasonFor(evToSalesValue, enterpriseValue !== null));
  const priceToOcf = await write('priceToOcf', priceToOcfValue, nullReasonFor(priceToOcfValue, marketCap !== null));
  const debtToFcf = await write('debtToFcf', debtToFcfValue, nullReasonFor(debtToFcfValue, totalDebt !== null));
  const capexToOcfRatio = await write('capexToOcfRatio', capexToOcfRatioValue, nullReasonFor(capexToOcfRatioValue, true));
  const croic = await write('croic', croicValue, nullReasonFor(croicValue, investedCapital !== null));
  const ocfMargin = await write('ocfMargin', ocfMarginValue, nullReasonFor(ocfMarginValue, true));
  const fcfConversionRate = await write('fcfConversionRate', fcfConversionRateValue, nullReasonFor(fcfConversionRateValue, true));

  return { symbol, rocYear: year, season, evToOcf, evToSales, priceToOcf, capexToOcfRatio, debtToFcf, croic, ocfMargin, fcfConversionRate };
};
