...
import { calculateDepreciationAmortizationPerShare } from '@/domainPitMetrics/quality/depreciationAmortizationPerShare/calculateDepreciationAmortizationPerShare';
...
export interface CashFlowPerSharePitOutcome extends QuarterlyPitOutcomeBase {
  ...
  depreciationAmortizationPerShareQ: BasisOutcome;
  depreciationAmortizationPerShareTtm: BasisOutcome;
}
...
export const computeAndWriteCashFlowPerSharePit = async (
  ...
  const skippedNoQuarter: CashFlowPerSharePitOutcome = {
    ...
    depreciationAmortizationPerShareQ: { action: 'skipped_no_quarter' },
    depreciationAmortizationPerShareTtm: { action: 'skipped_no_quarter' },
  };
  ...
  let depreciationAmortizationPerShareQ: BasisOutcome;
  ...
  if (!mainAnchor) {
    ...
    depreciationAmortizationPerShareQ = await writeMetricValue({ ...coordinateFor('depreciationAmortizationPerShare'), ...periodTypeGroup('Q'), value: depreciationAmortizationPerShareQuarterly.value, nullReason: depreciationAmortizationPerShareQuarterly.nullReason, knowledgeDate, knowledgeDateIsFallback });
  }
  ...
  let depreciationAmortizationPerShareTtm: BasisOutcome;
  ...
  if (ttmComplete) {
    ...
    if (!ttmAnchor) {
      ...
      depreciationAmortizationPerShareTtm = await writeMetricValue({ ...coordinateFor('depreciationAmortizationPerShare'), ...periodTypeGroup('TTM'), value: depreciationAmortizationPerShareTtmCalc.value, nullReason: depreciationAmortizationPerShareTtmCalc.nullReason, knowledgeDate, knowledgeDateIsFallback });
    }
  } else if (mainAnchor) {
    const { knowledgeDate, isFallback: knowledgeDateIsFallback } = mainAnchor;
    ...
    depreciationAmortizationPerShareTtm = await writeMetricValue({ ...coordinateFor('depreciationAmortizationPerShare'), ...periodTypeGroup('TTM'), value: null, nullReason: 'insufficient_history', knowledgeDate, knowledgeDateIsFallback });
  } else {
    ocfPerShareTtm = { action: 'skipped_no_knowledge_date' };
    fcfPerShareTtm = { action: 'skipped_no_knowledge_date' };
    depreciationAmortizationPerShareTtm = { action: 'skipped_no_knowledge_date' };
  ...
  return {
    ...
    fcfPerShareQ,
    fcfPerShareTtm,
    depreciationAmortizationPerShareQ,
    depreciationAmortizationPerShareTtm,
  };
};
