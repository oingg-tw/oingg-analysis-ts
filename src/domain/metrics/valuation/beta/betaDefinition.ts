...
export const betaDefinition: MetricDefinitionSpec = {
  ...
  formulaNote:
    ...
    '（分母 n-1）。四個 (lookbackRange, samplingInterval) 組合各自獨立計算（各自取基準' +
    '交易日往前 N 年的重疊交易日再降頻，不是用短窗口的資料湊長窗口）：1Y×1D 用日資料、' +
    '2Y×1W 用週資料（對齊 Bloomberg）、3Y×1W 用週資料（2026-09-15 新增，介於 Bloomberg ' +
    '2年週頻跟 Morningstar 5年月頻之間的折衷週期）、5Y×1M 用月資料（對齊 Morningstar/' +
    'S&P），降頻取「每個週期最後一個重疊交易日」代表。降頻後取樣點數 < 20 為 ' +
    'insufficient_history；指數變異數為 0（理論上不會發生但防呆）為 ' +
  ...
  group: 'rollingWindow',
  allowedLookbackRanges: ['1Y', '2Y', '3Y', '5Y'],
  allowedSamplingIntervals: ['1D', '1W', '1M'],
  // allowedLookbackRanges x allowedSamplingIntervals 上面兩欄不是自由交叉組合（4x3=12），
  // 四個窗口各自綁定固定的取樣頻率（1Y 用日頻、2Y/3Y 用週頻、5Y 用月頻，理由見上面
  // formulaNote），只有這 4 種組合真的會寫入資料——這個陣列是外部消費端（GET /metrics、
  // screener/companies 端點的 timeframe 驗證）唯一該信任的合法組合來源，2026-09-08 bff-ts
  // 拿真實資料實測 9 種組合才發現另外 6 種「查得到但永遠是空結果」，回報後補上。
  allowedRollingWindowTimeframes: ['1Y_1D', '2Y_1W', '3Y_1W', '5Y_1M'],
