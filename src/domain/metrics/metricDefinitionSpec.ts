...
export interface MetricBadge extends NamedEntity {
  ...
  // 衍生，各自手動維護避免格式耦合。
  author: string;
  // 2026-09-20 新增：**點進去就看得到這個門檻數字**的公開頁面。使用者回報「徽章連結點過去根本
  // 沒看到公式或門檻」，根因是這個型別原本沒有自己的連結欄位——前端只能拿指標的
  // referenceUrl/academicSourceUrl 來顯示，但那兩個欄位的用途是「這支指標是什麼、公式怎麼算」
  // （例如維基「毛利率」條目），不是「為什麼門檻是 40%」，所以點過去看不到門檻是結構必然。
  //
  // 這個欄位的契約比 referenceUrl 嚴格：**必須是實際打開確認過、頁面上真的有逐字寫出這個門檻
  // 數字的公開網址**，不是「相關概念的參考頁」，也不是只有書目資訊的付費牆論文頁（那種放
  // academicSourceUrl，用途是給想找原始論文的人，不是給一般使用者查門檻）。填進來的每一個
  // URL 都經過實際 fetch 驗證，驗證結果記在各 badge 檔案的註解裡。
  //
  // 選填：門檻出處是實體書、沒有合法免費全文可連的情況（例如 grossMargin/netProfitMargin 的
  // 門檻出自 Mary Buffett & David Clark 2008 那本書）留空，**留空不代表門檻是自訂的**
  // （出處仍然真實可指名，記在 author），只代表沒有可以點的公開頁面——前端這種情況不要渲染
  // 連結，不要退回去拿指標的 referenceUrl 充數，那正是這次要修掉的問題。
  sourceUrl?: string;
  summary: string;
  detail: string;
...
interface MetricDefinitionSpecBase extends NamedEntity {
  ...
  // 的 @/models/* 模組（或直接查詢的 DB 表）人工核對過的結果，不是從
