...
- `domains/metrics/growth/`：目前只有 `README.md` 記錄這一類要放哪些指標，還沒有任何程式碼——這是刻意的，先把分類骨架跟每個指標的公式/口徑記下來，之後要做哪個再回頭建 domain。這是另一套評估中的分類方案之一，見 [`src/domainBatch/metrics/README.md`](src/domainBatch/metrics/README.md) 的「第二套分類方案」說明（`securityInfo` 2026-09-02、`marketData`/`financials` 2026-09-05 已經從這份骨架清單移除，見該文件）。
