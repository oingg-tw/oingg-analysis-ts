...
describe('getCompanyDividendHistory', () => {
  ...
  test('年配公司多年：舊 → 新排序；沒有年報時 EPS 為 null、EPS ≤ 0 時 payoutRatio 為 null、查無股價時殖利率為 null', async () => {
    const rows = [
      ...
      row({ rocFiscalYear: 111, cashDividendFromEarnings: 1.2, cashDividendFromLegalReserveAndCapitalSurplus: 0.3, exDividendDate: day('2023-07-01') }),
    ...
    expect(entries[0]).toMatchObject({ cashDividend: 1.5, cashDividendFromEarnings: 1.2, cashDividendFromLegalReserveAndCapitalSurplus: 0.3, eps: 4, payoutRatio: 30, yieldAtExDate: 5 });
    expect(entries[0]!.events[0]).toMatchObject({ cashDividend: 1.5, cashDividendFromEarnings: 1.2, cashDividendFromLegalReserveAndCapitalSurplus: 0.3 });
  ...
  test('現金股利全部來自法定盈餘公積／資本公積 → payoutRatio 是 0（盈餘分配一毛沒發），不是合計 ÷ EPS', async () => {
    ...
    const rows = [row({ rocFiscalYear: 111, cashDividendFromLegalReserveAndCapitalSurplus: 0.9, exDividendDate: day('2023-07-01') })];
    const deps = depsFor(rows, { '2022': 2.58 }, {});
    const { entries } = await getCompanyDividendHistory('2882', deps);
    expect(entries[0]).toMatchObject({ cashDividend: 0.9, cashDividendFromEarnings: 0, cashDividendFromLegalReserveAndCapitalSurplus: 0.9, eps: 2.58, payoutRatio: 0 });
  });
});
