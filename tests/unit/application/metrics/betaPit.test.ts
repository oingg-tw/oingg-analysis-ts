...
const replay = createPitReplay('betaPit');
...
test('betaPit: 2330 四個窗口都應該算出合理範圍內的值，並正確寫入 metric_daily_cadence_values', async () => {
  const outcome = await replay.run(computeBeta)({ symbol: '2330', dataType: '2', subsidiaryCompanyId: '' });
  ...
  for (const [lookbackRange, samplingInterval] of [
    ...
    const row = await replay.findLatest({ symbol: '2330', metricCode: 'beta', lookbackRange, samplingInterval, snapshotCadence: 'N/A', dataType: '2', subsidiaryCompanyId: '' });
...
test('betaPit: 重跑同一個基準日，去重邏輯應該讓第二次全部 skipped_unchanged', async () => {
  ...
  await replay.run(computeBeta)(query);
  const second = await replay.run(computeBeta)(query);
...
test('betaPit: 9999（查無股價資料的公司）應該優雅降級，不寫入', async () => {
  const outcome = await replay.run(computeBeta)({ symbol: '9999', dataType: '2', subsidiaryCompanyId: '' });
  ...
  const count = await replay.count({ symbol: '9999', metricCode: 'beta' });
