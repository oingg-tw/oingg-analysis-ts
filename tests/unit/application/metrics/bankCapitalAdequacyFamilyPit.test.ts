...
const replay = createPitReplay('bankCapitalAdequacyFamilyPit');
...
test('bankCapitalAdequacyFamilyPit: 2801（彰化銀行）115Q2（有揭露的季度），跟實測驗證過的真實數字交叉驗證', async () => {
  await replay.run(computeBankCapitalAdequacyFamily)({ symbol: '2801', year: '115', season: '2', dataType: '2', subsidiaryCompanyId: '' });

  const where = { symbol: '2801', fiscalYear: 2026, fiscalQuarter: 2, dataType: '2', subsidiaryCompanyId: '' };
  const car = await replay.findLatest({ ...where, metricCode: 'bankCarRatio', periodType: 'Q' });
  const cet1 = await replay.findLatest({ ...where, metricCode: 'bankCet1Ratio', periodType: 'Q' });
  const tier1 = await replay.findLatest({ ...where, metricCode: 'bankTier1Ratio', periodType: 'Q' });
...
test('bankCapitalAdequacyFamilyPit: 2801 115Q1（監理揭露半年一次，這季本來就沒揭露），應該優雅降級成 missing_input', async () => {
  await replay.run(computeBankCapitalAdequacyFamily)({ symbol: '2801', year: '115', season: '1', dataType: '2', subsidiaryCompanyId: '' });
...
test('bankCapitalAdequacyFamilyPit: 不給 year/season 時自動抓最新一季，應該跳過值為 null 的季度', async () => {
  const outcome = await replay.run(computeBankCapitalAdequacyFamily)({ symbol: '2801', dataType: '2', subsidiaryCompanyId: '' });
...
test('bankCapitalAdequacyFamilyPit: 2330（台積電，真實存在但不是銀行）應該完全不寫入任何列', async () => {
  ...
  const outcome = await replay.run(computeBankCapitalAdequacyFamily)({ symbol: '2330', year: '115', season: '2', dataType: '2', subsidiaryCompanyId: '' });
  ...
  const car = await replay.findLatest(where);
