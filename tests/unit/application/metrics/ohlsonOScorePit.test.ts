...
const replay = createPitReplay('ohlsonOScorePit');
...
test(
  ...
  async () => {
    await replay.run(computeOhlsonOScore)({ symbol: '2330', year: '115', season: '2', dataType: '2', subsidiaryCompanyId: '' });

    const ttm = await replay.findLatest({ symbol: '2330', metricCode: 'ohlsonOScore', periodType: 'TTM', fiscalYear: 2026, fiscalQuarter: 2, dataType: '2', subsidiaryCompanyId: '' });
...
test('ohlsonOScorePit: 9999（查無資料的公司）應該優雅降級，不寫入', async () => {
  const outcome = await replay.run(computeOhlsonOScore)({ symbol: '9999', dataType: '2', subsidiaryCompanyId: '' });

  assert.deepEqual(outcome.ttm, { action: 'skipped_no_quarter' });
  const count = await replay.count({ symbol: '9999', metricCode: 'ohlsonOScore' });
