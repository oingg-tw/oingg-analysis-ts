...
const replay = createPitReplay('roaPit');
...
test('roaPit: 2330 115Q2 合併報表，跟 roa.test.ts 的既有基準數字交叉驗證', async () => {
  await replay.run(computeRoa)({ symbol: '2330', year: '115', season: '2', dataType: '2', subsidiaryCompanyId: '' });

  const findLatest = (periodType: string) =>
    replay.findLatest({ symbol: '2330', metricCode: 'roa', periodType, fiscalYear: 2026, fiscalQuarter: 2, dataType: '2', subsidiaryCompanyId: '' });
...
test('roaPit: 重跑同一組座標，去重邏輯應該讓第二次全部 skipped_unchanged，且列數維持 1', async () => {
  await replay.run(computeRoa)({ symbol: '2887', year: '115', season: '1', dataType: '2', subsidiaryCompanyId: '' });
  const second = await replay.run(computeRoa)({ symbol: '2887', year: '115', season: '1', dataType: '2', subsidiaryCompanyId: '' });
  ...
  const count = await replay.count({ symbol: '2887', metricCode: 'roa', periodType: 'Q', fiscalYear: 2026, fiscalQuarter: 1, dataType: '2', subsidiaryCompanyId: '' });
...
test('roaPit: 2317 115Q2——financial_report_announcement 無覆蓋，knowledge_date 應該標記為 fallback', async () => {
  const outcome = await replay.run(computeRoa)({ symbol: '2317', year: '115', season: '2', dataType: '2', subsidiaryCompanyId: '' });

  assert.notEqual(outcome.q, undefined);
  const q = await replay.findLatest({ symbol: '2317', metricCode: 'roa', periodType: 'Q', fiscalYear: 2026, fiscalQuarter: 2, dataType: '2', subsidiaryCompanyId: '' });
...
test('roaPit: 2317 115Q2 的 TTM 換源後（XBRL 補齊 114Q4）應該算得出真實數字，不再是 insufficient_history', async () => {
  await replay.run(computeRoa)({ symbol: '2317', year: '115', season: '2', dataType: '2', subsidiaryCompanyId: '' });

  const ttm = await replay.findLatest({ symbol: '2317', metricCode: 'roa', periodType: 'TTM', fiscalYear: 2026, fiscalQuarter: 2, dataType: '2', subsidiaryCompanyId: '' });
...
test('roaPit: 9999（查無資料的公司）應該優雅降級，都不寫入', async () => {
  const outcome = await replay.run(computeRoa)({ symbol: '9999', dataType: '2', subsidiaryCompanyId: '' });
  ...
  const count = await replay.count({ symbol: '9999', metricCode: 'roa' });
