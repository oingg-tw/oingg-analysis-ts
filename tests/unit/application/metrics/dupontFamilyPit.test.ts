...
const replay = createPitReplay('dupontFamilyPit');
...
const findLatest = (symbol: string, metricCode: string, periodType: string, fiscalYear: number, fiscalQuarter: number) =>
  replay.findLatest({ symbol, metricCode, periodType, fiscalYear, fiscalQuarter, dataType: '2', subsidiaryCompanyId: '' });
...
test('dupontFamilyPit: 重跑同一組座標，去重邏輯應該讓第二次全部 skipped_unchanged，且列數維持 1', async () => {
  await replay.run(computeDupontFamily)({ symbol: '2887', year: '115', season: '1', dataType: '2', subsidiaryCompanyId: '' });
  const second = await replay.run(computeDupontFamily)({ symbol: '2887', year: '115', season: '1', dataType: '2', subsidiaryCompanyId: '' });
  ...
  const count = await replay.count({ symbol: '2887', metricCode: 'dupontDecomposedRoe', periodType: 'Q', fiscalYear: 2026, fiscalQuarter: 1, dataType: '2', subsidiaryCompanyId: '' });
...
test('dupontFamilyPit: 2317 115Q2——financial_report_announcement 無覆蓋，knowledge_date 應該標記為 fallback', async () => {
  const outcome = await replay.run(computeDupontFamily)({ symbol: '2317', year: '115', season: '2', dataType: '2', subsidiaryCompanyId: '' });
...
test('dupontFamilyPit: 2317 115Q2 的 TTM 換源後（XBRL 補齊 114Q4）應該算得出真實數字，不再是 insufficient_history', async () => {
  await replay.run(computeDupontFamily)({ symbol: '2317', year: '115', season: '2', dataType: '2', subsidiaryCompanyId: '' });
...
test('dupontFamilyPit: 9999（查無資料的公司）應該優雅降級，全部 metric_code 都不寫入', async () => {
  const outcome = await replay.run(computeDupontFamily)({ symbol: '9999', dataType: '2', subsidiaryCompanyId: '' });
  ...
  const count = await replay.count({ symbol: '9999', metricCode: { in: ['netProfitMargin', 'assetTurnover', 'equityMultiplier', 'dupontDecomposedRoe'] } });
