...
const replay = createPitReplay('bankIncomeWaterfallPit');
...
test('bankIncomeWaterfallPit: 2801 114Q2 應該寫入四個 metric_code，且瀑布圖恆等式精確成立', async () => {
  await replay.run(computeBankIncomeWaterfall)({ symbol: '2801', year: '114', season: '2', dataType: '2', subsidiaryCompanyId: '' });
  await replay.run(computePretaxIncomePerShare)({ symbol: '2801', year: '114', season: '2', dataType: '2', subsidiaryCompanyId: '' });
  ...
  const findLatest = (metricCode: string, periodType: string) =>
    replay.findLatest({ symbol: '2801', metricCode, periodType, fiscalYear: 2025, fiscalQuarter: 2, dataType: '2', subsidiaryCompanyId: '' });
  ...
  const pretaxQ = await replay.findLatest({ symbol: '2801', metricCode: 'pretaxIncomePerShare', periodType: 'Q', fiscalYear: 2025, fiscalQuarter: 2, dataType: '2', subsidiaryCompanyId: '' });
...
test('bankIncomeWaterfallPit: 2330（非銀行公司）應該優雅降級，不寫入任何一列', async () => {
  const outcome = await replay.run(computeBankIncomeWaterfall)({ symbol: '2330', dataType: '2', subsidiaryCompanyId: '' });

  assert.deepEqual(outcome.bankNetInterestIncomePerShareQ, { action: 'skipped_no_quarter' });
  const count = await replay.count({ symbol: '2330', metricCode: { in: ['bankNetInterestIncomePerShare', 'bankNetNonInterestIncomePerShare', 'bankBadDebtProvisionPerShare', 'bankOtherOperatingExpensePerShare'] } });
