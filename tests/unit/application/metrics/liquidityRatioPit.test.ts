...
const replay = createPitReplay('liquidityRatioPit');

test('liquidityRatioPit: 2330 115Q2 合併報表，跟 liquidityRatio.test.ts 的既有基準數字交叉驗證', async () => {
  await replay.run(computeLiquidityRatio)({ symbol: '2330', year: '115', season: '2', dataType: '2', subsidiaryCompanyId: '' });

  const findLatest = (metricCode: string) =>
    replay.findLatest({ symbol: '2330', metricCode, periodType: 'Q', fiscalYear: 2026, fiscalQuarter: 2, dataType: '2', subsidiaryCompanyId: '' });
...
test('liquidityRatioPit: 9999（查無資料的公司）應該優雅降級，不寫入', async () => {
  const outcome = await replay.run(computeLiquidityRatio)({ symbol: '9999', dataType: '2', subsidiaryCompanyId: '' });

  assert.deepEqual(outcome.currentRatio, { action: 'skipped_no_quarter' });
  const count = await replay.count({ symbol: '9999', metricCode: { in: ['currentRatio', 'quickRatio', 'cashRatio'] } });
