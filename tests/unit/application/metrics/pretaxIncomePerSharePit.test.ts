...
const replay = createPitReplay('pretaxIncomePerSharePit');
...
test('pretaxIncomePerSharePit: 2330 115Q2 合併報表，稅前淨利應大於同期 EPS（稅後）', async () => {
  await replay.run(computePretaxIncomePerShare)({ symbol: '2330', year: '115', season: '2', dataType: '2', subsidiaryCompanyId: '' });

  const findLatest = (periodType: string) =>
    replay.findLatest({ symbol: '2330', metricCode: 'pretaxIncomePerShare', periodType, fiscalYear: 2026, fiscalQuarter: 2, dataType: '2', subsidiaryCompanyId: '' });
...
test('pretaxIncomePerSharePit: 9999（查無資料的公司）應該優雅降級，都不寫入', async () => {
  const outcome = await replay.run(computePretaxIncomePerShare)({ symbol: '9999', dataType: '2', subsidiaryCompanyId: '' });

  assert.deepEqual(outcome.q, { action: 'skipped_no_quarter' });
  const count = await replay.count({ symbol: '9999', metricCode: 'pretaxIncomePerShare' });
