...
const replay = createPitReplay('peRatioPit');
...
test('peRatioPit: 2330 115Q2，用真實股價/EPS_TTM 手動核算過的基準數字交叉驗證', async () => {
  const outcome = await replay.run(computePeRatio)({ symbol: '2330', year: '115', season: '2', dataType: '2', subsidiaryCompanyId: '' });

  const ttm = await replay.findLatest({ symbol: '2330', metricCode: 'peRatio', periodType: 'TTM', fiscalYear: 2026, fiscalQuarter: 2, dataType: '2', subsidiaryCompanyId: '' });
...
test('peRatioPit: 重跑同一組座標，去重邏輯應該讓第二次全部 skipped_unchanged，且列數維持 1', async () => {
  ...
  await replay.run(computePeRatio)(query);
  const second = await replay.run(computePeRatio)(query);
  ...
  const count = await replay.count({ symbol: '2330', metricCode: 'peRatio', periodType: 'TTM', fiscalYear: 2026, fiscalQuarter: 1, dataType: '2', subsidiaryCompanyId: '' });
  assert.equal(count, 1, '重複寫入同一個座標不應該疊加成多列');
});

test('peRatioPit: 9999（查無資料的公司）應該優雅降級，不寫入', async () => {
  const outcome = await replay.run(computePeRatio)({ symbol: '9999', dataType: '2', subsidiaryCompanyId: '' });

  assert.equal(outcome.rocYear, null);
  assert.equal(outcome.season, null);
  assert.deepEqual(outcome.ttm, { action: 'skipped_no_quarter' });

  const count = await replay.count({ symbol: '9999', metricCode: 'peRatio' });
