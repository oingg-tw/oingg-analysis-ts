...
const replay = createPitReplay('marketRatiosPit');
...
test('marketRatiosPit: 2330 應該把 daily_valuation 的三個欄位原封不動寫進 metric_daily_cadence_values', async () => {
  const outcome = await replay.run(computeMarketRatios)({ symbol: '2330', dataType: '2', subsidiaryCompanyId: '' });
  ...
  const [pe, pb, dy] = await Promise.all(
    ['exchangePeRatio', 'exchangePbRatio', 'dividendYield'].map((metricCode) =>
      replay.findLatest({ symbol: '2330', metricCode, lookbackRange: 'N/A', samplingInterval: 'N/A', snapshotCadence: 'EOD', dataType: '2', subsidiaryCompanyId: '' })
    )
  );
...
test('marketRatiosPit: 重跑同一天，去重邏輯應該讓第二次全部 skipped_unchanged', async () => {
  ...
  const first = await replay.run(computeMarketRatios)(query);
  const second = await replay.run(computeMarketRatios)(query);
...
test('marketRatiosPit: 9999（查無 daily_valuation 資料的公司）應該優雅降級，不寫入', async () => {
  const outcome = await replay.run(computeMarketRatios)({ symbol: '9999', dataType: '2', subsidiaryCompanyId: '' });
  ...
  const count = await replay.count({ symbol: '9999', metricCode: { in: ['exchangePeRatio', 'exchangePbRatio', 'dividendYield'] } });
