...
const replay = createPitReplay('bankAssetQualityFamilyPit');
...
test('bankAssetQualityFamilyPit: 2801（彰化銀行）115Q2，跟實測驗證過的真實數字交叉驗證', async () => {
  await replay.run(computeBankAssetQualityFamily)({ symbol: '2801', year: '115', season: '2', dataType: '2', subsidiaryCompanyId: '' });

  const where = { symbol: '2801', fiscalYear: 2026, fiscalQuarter: 2, dataType: '2', subsidiaryCompanyId: '' };
  const npl = await replay.findLatest({ ...where, metricCode: 'bankNplRatio', periodType: 'Q' });
  const coverage = await replay.findLatest({ ...where, metricCode: 'bankNplCoverageRatio', periodType: 'Q' });
...
test('bankAssetQualityFamilyPit: 2330（台積電，真實存在但不是銀行）應該完全不寫入任何列', async () => {
  ...
  const outcome = await replay.run(computeBankAssetQualityFamily)({ symbol: '2330', year: '115', season: '2', dataType: '2', subsidiaryCompanyId: '' });
  ...
  const npl = await replay.findLatest(where);
  assert.equal(npl, null);
});

test('bankAssetQualityFamilyPit: 9999（查無資料的公司）應該優雅降級，不寫入', async () => {
  const outcome = await replay.run(computeBankAssetQualityFamily)({ symbol: '9999', dataType: '2', subsidiaryCompanyId: '' });

  assert.deepEqual(outcome.bankNplRatio, { action: 'skipped_no_quarter' });
  const count = await replay.count({ symbol: '9999', metricCode: 'bankNplRatio' });
