...
const replay = createPitReplay('roePit');
...
//
// 2026-09-17 Phase 3：原本這裡的「塞假 FinancialStatementPort 驗證負權益」DIP 示範案例改寫成
// tests/unit/application/metrics/profitability/roe/computeRoe.test.ts（記憶體 port，完全不連資料庫）；
// 這支只留下釘真實資料數字的案例。
...
test('roePit: 2330 115Q2 合併報表，跟 roe.test.ts 的既有基準數字交叉驗證', async () => {
  ...
  const findLatest = (periodType: string) =>
    replay.findLatest({ symbol: '2330', metricCode: 'roe', periodType, fiscalYear: 2026, fiscalQuarter: 2, dataType: '2', subsidiaryCompanyId: '' });
...
test('roePit: 重跑同一組座標，去重邏輯應該讓第二次全部 skipped_unchanged，且列數維持 1', async () => {
  await replay.run(computeRoe)({ symbol: '2887', year: '115', season: '1', dataType: '2', subsidiaryCompanyId: '' });
  const second = await replay.run(computeRoe)({ symbol: '2887', year: '115', season: '1', dataType: '2', subsidiaryCompanyId: '' });
  ...
  const count = await replay.count({ symbol: '2887', metricCode: 'roe', periodType: 'Q', fiscalYear: 2026, fiscalQuarter: 1, dataType: '2', subsidiaryCompanyId: '' });
...
test('roePit: 2317 115Q2——financial_report_announcement 無覆蓋，knowledge_date 應該標記為 fallback', async () => {
  const outcome = await replay.run(computeRoe)({ symbol: '2317', year: '115', season: '2', dataType: '2', subsidiaryCompanyId: '' });

  assert.notEqual(outcome.q, undefined);
  const q = await replay.findLatest({ symbol: '2317', metricCode: 'roe', periodType: 'Q', fiscalYear: 2026, fiscalQuarter: 2, dataType: '2', subsidiaryCompanyId: '' });
...
test('roePit: 2317 115Q2 的 TTM 換源後（XBRL 補齊 114Q4）應該算得出真實數字，不再是 insufficient_history', async () => {
  await replay.run(computeRoe)({ symbol: '2317', year: '115', season: '2', dataType: '2', subsidiaryCompanyId: '' });

  const ttm = await replay.findLatest({ symbol: '2317', metricCode: 'roe', periodType: 'TTM', fiscalYear: 2026, fiscalQuarter: 2, dataType: '2', subsidiaryCompanyId: '' });
...
test('roePit: 9999（查無資料的公司）應該優雅降級，都不寫入', async () => {
  const outcome = await replay.run(computeRoe)({ symbol: '9999', dataType: '2', subsidiaryCompanyId: '' });
  ...
  const count = await replay.count({ symbol: '9999', metricCode: 'roe' });
