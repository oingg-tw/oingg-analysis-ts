...
const replay = createPitReplay('leverageDegreeFamilyPit');
...
test('leverageDegreeFamilyPit: 2330 DFL/DTL 都應該算出非 null 的 Q 值', async () => {
  const outcome = await replay.run(computeLeverageDegreeFamily)({ symbol: '2330', dataType: '2', subsidiaryCompanyId: '' });
  assert.notEqual(outcome.rocYear, null);

  const rows = await replay.findMany({ symbol: '2330', metricCode: { in: CODES }, periodType: 'Q' });
...
test('leverageDegreeFamilyPit: 重跑同一季，去重邏輯應該讓第二次 skipped_unchanged', async () => {
  ...
  await replay.run(computeLeverageDegreeFamily)(query);
  const second = await replay.run(computeLeverageDegreeFamily)(query);
...
test('leverageDegreeFamilyPit: 9999（查無資料的公司）應該優雅降級', async () => {
  const outcome = await replay.run(computeLeverageDegreeFamily)({ symbol: '9999', dataType: '2', subsidiaryCompanyId: '' });
