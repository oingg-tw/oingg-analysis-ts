...
const replay = createPitReplay('marginsFamilyPit');
...
test('marginsFamilyPit: 2330 115Q2 合併報表，跟既有基準數字交叉驗證', async () => {
  ...
  const grossQ = await replay.findLatest({ symbol: '2330', metricCode: 'grossMargin', periodType: 'Q', fiscalYear: 2026, fiscalQuarter: 2, dataType: '2', subsidiaryCompanyId: '' });
  const operatingQ = await replay.findLatest({ symbol: '2330', metricCode: 'operatingMargin', periodType: 'Q', fiscalYear: 2026, fiscalQuarter: 2, dataType: '2', subsidiaryCompanyId: '' });
...
test('marginsFamilyPit: 2851（中再保，legacy 表完全無資料）應該 fallback 到保險替代科目', async () => {
  const outcome = await replay.run(computeMarginsFamily)({ symbol: '2851', dataType: '2', subsidiaryCompanyId: '' });

  assert.notEqual(outcome.rocYear, null, '應該能透過保險替代來源解析出最新一季，不是 skipped_no_quarter');

  const grossQ = await replay.findLatest({ symbol: '2851', metricCode: 'grossMargin', periodType: 'Q' });
  const operatingQ = await replay.findLatest({ symbol: '2851', metricCode: 'operatingMargin', periodType: 'Q' });
...
test('marginsFamilyPit: 2867（三商美邦人壽，合併報表兩層來源都無法解析季度）應該優雅降級，不寫入', async () => {
  const outcome = await replay.run(computeMarginsFamily)({ symbol: '2867', dataType: '2', subsidiaryCompanyId: '' });

  assert.equal(outcome.rocYear, null);
  assert.deepEqual(outcome.grossMarginQ, { action: 'skipped_no_quarter' });

  const count = await replay.count({ symbol: '2867', metricCode: { in: ['grossMargin', 'operatingMargin'] } });
  assert.equal(count, 0);
});

test('marginsFamilyPit: 9999（查無資料的公司）應該優雅降級，不寫入', async () => {
  const outcome = await replay.run(computeMarginsFamily)({ symbol: '9999', dataType: '2', subsidiaryCompanyId: '' });

  assert.deepEqual(outcome.grossMarginQ, { action: 'skipped_no_quarter' });
  const count = await replay.count({ symbol: '9999', metricCode: { in: ['grossMargin', 'operatingMargin'] } });
  assert.equal(count, 0);
});
