...
const replay = createPitReplay('stockPricePit');
...
test('stockPricePit: 2330 115Q2，價格應該精確等於 knowledge_date 當天收盤價（跟 pbRatio 用的同一個數字）', async () => {
  const outcome = await replay.run(computeStockPrice)({ symbol: '2330', year: '115', season: '2', dataType: '2', subsidiaryCompanyId: '' });

  const q = await replay.findLatest({ symbol: '2330', metricCode: 'stockPrice', periodType: 'Q', fiscalYear: 2026, fiscalQuarter: 2, dataType: '2', subsidiaryCompanyId: '' });
...
test('stockPricePit: 重跑同一組座標，去重邏輯應該讓第二次 skipped_unchanged，且列數維持 1', async () => {
  ...
  await replay.run(computeStockPrice)(query);
  const second = await replay.run(computeStockPrice)(query);

  assert.deepEqual(second.q, { action: 'skipped_unchanged' });

  const count = await replay.count({ symbol: '2330', metricCode: 'stockPrice', periodType: 'Q', fiscalYear: 2026, fiscalQuarter: 1, dataType: '2', subsidiaryCompanyId: '' });
  assert.equal(count, 1, '重複寫入同一個座標不應該疊加成多列');
});

test('stockPricePit: 9999（查無資料的公司）應該優雅降級，不寫入', async () => {
  const outcome = await replay.run(computeStockPrice)({ symbol: '9999', dataType: '2', subsidiaryCompanyId: '' });

  assert.equal(outcome.rocYear, null);
  assert.equal(outcome.season, null);
  assert.deepEqual(outcome.q, { action: 'skipped_no_quarter' });

  const count = await replay.count({ symbol: '9999', metricCode: 'stockPrice' });
