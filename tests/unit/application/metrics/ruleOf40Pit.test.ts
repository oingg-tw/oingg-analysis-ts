...
const replay = createPitReplay('ruleOf40Pit');
...
test('ruleOf40Pit: 3130（資訊服務業）115Q2，跟實測基準數字交叉驗證', async () => {
  await replay.run(computeRuleOf40)({ symbol: '3130', year: '115', season: '2', dataType: '2', subsidiaryCompanyId: '' });

  const ttm = await replay.findLatest({ symbol: '3130', metricCode: 'ruleOf40', periodType: 'TTM', fiscalYear: 2026, fiscalQuarter: 2, dataType: '2', subsidiaryCompanyId: '' });
...
test('ruleOf40Pit: 2330（非資訊服務業/數位雲端）應該優雅降級，不寫入任何列', async () => {
  const outcome = await replay.run(computeRuleOf40)({ symbol: '2330', dataType: '2', subsidiaryCompanyId: '' });

  assert.deepEqual(outcome.ttm, { action: 'skipped_no_quarter' });
  const count = await replay.count({ symbol: '2330', metricCode: 'ruleOf40' });
  assert.equal(count, 0);
});

test('ruleOf40Pit: 9999（查無資料的公司）應該優雅降級，不寫入', async () => {
  const outcome = await replay.run(computeRuleOf40)({ symbol: '9999', dataType: '2', subsidiaryCompanyId: '' });

  assert.deepEqual(outcome.ttm, { action: 'skipped_no_quarter' });
  const count = await replay.count({ symbol: '9999', metricCode: 'ruleOf40' });
  assert.equal(count, 0);
});
