...
import { runEtfScreener, getEtfFilterCatalog, EtfScreenerValidationError } from '@/application/market/etfScreener/service';
...
test('runEtfScreener: 數字 filter 應該只保留落在範圍內的值，null 排除', async () => {
  const result = await runEtfScreener({ filters: [{ field: 'aum', min: 10_000_000_000, max: null }], columns: [{ field: 'aum' }], page: 1, pageSize: 50 }, appDeps);
...
test('runEtfScreener: exclude=true 應該保留範圍外的值', async () => {
  const result = await runEtfScreener({ filters: [{ field: 'return1y', min: 0, max: 50, exclude: true }], columns: [{ field: 'return1y' }], page: 1, pageSize: 50 }, appDeps);
  for (const row of result.results) {
...
test('runEtfScreener: distributionFrequency 的值不應該還帶著括號（正則跳脫要用雙反斜線）', async () => {
  const result = await runEtfScreener({ filters: [{ field: 'distributionFrequency', values: ['月配'] }], columns: [{ field: 'distributionFrequency' }], page: 1, pageSize: 50 }, appDeps);
...
test('getEtfFilterCatalog: 應該回傳五個非空分類，每個欄位都恰好歸類在一個分類裡', async () => {
  const catalog = await getEtfFilterCatalog(appDeps);
...
test('runEtfScreener: 數字欄位給 values 應該拋 EtfScreenerValidationError', async () => {
  await assert.rejects(
    runEtfScreener({ filters: [{ field: 'aum', values: ['1'] } as never], columns: [], page: 1, pageSize: 50 }, appDeps),
...
test('runEtfScreener: 類別欄位給 min/max 應該拋 EtfScreenerValidationError', async () => {
  await assert.rejects(
    runEtfScreener({ filters: [{ field: 'market', min: 0, max: 1 } as never], columns: [], page: 1, pageSize: 50 }, appDeps),
    EtfScreenerValidationError
  );
});

test('runEtfScreener: 查不到的 field 應該拋 EtfScreenerValidationError', async () => {
  await assert.rejects(runEtfScreener({ filters: [], columns: [{ field: 'notARealField' }], page: 1, pageSize: 50 }, appDeps), EtfScreenerValidationError);
});

test('runEtfScreener: sortField 不在 columns 裡應該拋 EtfScreenerValidationError', async () => {
  await assert.rejects(
    runEtfScreener({ filters: [], columns: [{ field: 'aum' }], page: 1, pageSize: 50, sortField: 'holders', sortOrder: 'desc' }, appDeps),
    EtfScreenerValidationError
  );
});
...
test('runEtfScreener: expenseRatio 的值應該只來自最新一個完整年度（發行日不滿一整年時是 null）', async () => {
  ...
  const result = await runEtfScreener({ filters: [], columns: [{ field: 'expenseRatio' }], page: 1, pageSize: 200 }, appDeps);
...
test('runEtfScreener: 分頁應該正確切頁不重複', async () => {
  const page1 = await runEtfScreener({ filters: [], columns: [{ field: 'aum' }], page: 1, pageSize: 50, sortField: 'symbol', sortOrder: 'asc' }, appDeps);
  const page2 = await runEtfScreener({ filters: [], columns: [{ field: 'aum' }], page: 2, pageSize: 50, sortField: 'symbol', sortOrder: 'asc' }, appDeps);
...
test('runEtfScreener: belowStatutoryThreshold 類別 filter 應該正確轉換布林值', async () => {
  const result = await runEtfScreener({ filters: [{ field: 'belowStatutoryThreshold', values: ['false'] }], columns: [{ field: 'belowStatutoryThreshold' }], page: 1, pageSize: 200 }, appDeps);
...
test('runEtfScreener: statutoryAumThreshold 數字欄位應該是可以查詢的數字', async () => {
  const result = await runEtfScreener({ filters: [], columns: [{ field: 'statutoryAumThreshold' }], page: 1, pageSize: 50 }, appDeps);
  assert.ok(result.results.length > 0);
...
test('runEtfScreener: expenseRatio2023 應該有值，跟舊的 expenseRatio 欄位互不影響', async () => {
  const result = await runEtfScreener({ filters: [], columns: [{ field: 'expenseRatio2023' }, { field: 'expenseRatio' }], page: 1, pageSize: 200 }, appDeps);
...
test('runEtfScreener: 太早的年度（例如 2001，該基金那年還沒成立）應該是 null，不是拋錯', async () => {
  const result = await runEtfScreener({ filters: [], columns: [{ field: 'expenseRatio2001' }], page: 1, pageSize: 200 }, appDeps);
  assert.ok(result.results.length > 0);
...
test('getEtfFilterCatalog: 每個數字欄位都不應該有 values', () => {
  return getEtfFilterCatalog(appDeps).then((catalog) => {
...
test('runEtfScreener: establishedDate 可以查詢，且可以用日期字串 min/max 篩選', async () => {
  const result = await runEtfScreener({ filters: [], columns: [{ field: 'establishedDate' }], page: 1, pageSize: 50 }, appDeps);
  assert.ok(result.results.length > 0);
