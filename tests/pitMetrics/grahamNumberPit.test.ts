...
import { computeAndWriteGrahamNumberPit } from '@/pitMetrics/grahamNumber/computeGrahamNumberPit';
...
test('grahamNumberPit: 2330 115Q2 合併報表（只有 TTM 口徑），跟既有基準數字交叉驗證', async () => {
  ...
  const ttm = await analysisPrisma.metricValue.findFirst({
    where: { symbol: '2330', metricCode: 'grahamNumber', basis: 'TTM', fiscalYear: 2026, fiscalQuarter: 2, dataType: '2', subsidiaryCompanyId: '' },
