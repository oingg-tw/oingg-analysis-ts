...
test('getDailyCadenceMetricHistory: 2330 beta 2Y/1W 應該回傳遠多於 7 筆（不是被 fiscalYear 誤判成一年一筆）', async () => {
  const result = await getDailyCadenceMetricHistory('2330', 'beta', { lookbackRange: '2Y', samplingInterval: '1W', snapshotCadence: 'N/A' }, '2', '', 40);
...
test('getDailyCadenceMetricHistory: 查無此公司時 entries 是空陣列、total=0、hasMore=false', async () => {
  const result = await getDailyCadenceMetricHistory('999999', 'beta', { lookbackRange: '2Y', samplingInterval: '1W', snapshotCadence: 'N/A' }, '2', '', 20);
