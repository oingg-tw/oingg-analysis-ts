...
import { computeAndWriteRocePit } from '@/pitMetrics/profitability/roce/computeRocePit';
...
test('rocePit: 2330 115Q2 合併報表，跟既有基準數字交叉驗證', async () => {
  ...
  const findLatest = (basis: string) =>
    analysisPrisma.metricValue.findFirst({
      where: { symbol: '2330', metricCode: 'roce', basis, fiscalYear: 2026, fiscalQuarter: 2, dataType: '2', subsidiaryCompanyId: '' },
