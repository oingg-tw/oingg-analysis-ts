...
import { computeAndWriteRoicPit } from '@/domainPitMetrics/profitability/roic/computeRoicPit';
...
test('roicPit: 2330 115Q2 合併報表，跟既有基準數字交叉驗證', async () => {
  ...
  const findLatest = (basis: string) =>
    analysisPrisma.metricValue.findFirst({
      where: { symbol: '2330', metricCode: 'roic', periodType: basis, fiscalYear: 2026, fiscalQuarter: 2, dataType: '2', subsidiaryCompanyId: '' },
  ...
  assert.equal(Number(qAnn!.value), 68.16);
