...
// 到 110Q3~115Q2 共 20 季（見 scripts/backfillPeRatioAndPbRatioPit.ts），拿來驗證
// total/hasMore 的行為。

test('getMetricHistory: limit 小於總期數時，entries 只回傳 limit 筆，total 是完整期數，hasMore=true', async () => {
  const result = await getMetricHistory('2330', 'bvps', 'Q', '2', '', 5);
  assert.equal(result.entries.length, 5, 'entries 應該只有 5 筆');
  assert.equal(result.total, 20, '2330 bvps 已知 backfill 到 20 季');
  assert.equal(result.hasMore, true, '20 > 5，應該還有更多資料');
});

test('getMetricHistory: limit 大於等於總期數時，entries 回傳全部，hasMore=false', async () => {
  const result = await getMetricHistory('2330', 'bvps', 'Q', '2', '', 40);
  assert.equal(result.entries.length, 20, '總共只有 20 季，limit=40 應該全部回傳');
  assert.equal(result.total, 20);
  assert.equal(result.hasMore, false, '20 <= 40，沒有更多資料了');
});

test('getMetricHistory: 查無任何資料時，total=0、hasMore=false、entries 是空陣列', async () => {
  const result = await getMetricHistory('999999', 'bvps', 'Q', '2', '', 20);
