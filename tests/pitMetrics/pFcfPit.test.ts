...
import { computeAndWritePFcfPit } from '@/pitMetrics/valuation/pFcf/computePFcfPit';
...
test('pFcfPit: 2330 115Q2 合併報表，寫入的值應該落在合理區間', async () => {
  ...
  const ttm = await analysisPrisma.metricValue.findFirst({
    where: { symbol: '2330', metricCode: 'pFcf', basis: 'TTM', fiscalYear: 2026, fiscalQuarter: 2, dataType: '2', subsidiaryCompanyId: '' },
