...
import { computeAndWriteCapexToRevenuePit } from '@/pitMetrics/efficiency/capexToRevenue/computeCapexToRevenuePit';
...
test('capexToRevenuePit: 2330 115Q2 合併報表，跟既有基準數字交叉驗證', async () => {
  ...
  const q = await analysisPrisma.metricValue.findFirst({
    where: { symbol: '2330', metricCode: 'capexToRevenue', basis: 'Q', fiscalYear: 2026, fiscalQuarter: 2, dataType: '2', subsidiaryCompanyId: '' },
    orderBy: { knowledgeDate: 'desc' },
  });
  const ttm = await analysisPrisma.metricValue.findFirst({
    where: { symbol: '2330', metricCode: 'capexToRevenue', basis: 'TTM', fiscalYear: 2026, fiscalQuarter: 2, dataType: '2', subsidiaryCompanyId: '' },
    orderBy: { knowledgeDate: 'desc' },
  });
