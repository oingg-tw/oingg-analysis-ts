...
import { loadIndustryClassification, findPeerGroup, getIndustryNodeInfo, listIndustryChildren, listIndustryCompanies } from '@/shared/sourceData/industryClassification';
...
// 產業階層瀏覽（純瀏覽，不做動態回退）——跟上面 findPeerGroup 的測試是兩種不同語意的查詢。
...
test('listIndustryChildren/listIndustryCompanies: 查到 subclass 層級，children 是空陣列，companies 有實際公司', () => {
  ...
  assert.equal(info?.companyCount, 18, '實測 subclass 2711-00 有 18 家公司，跟 findPeerGroup 的 7711 測試案例一致');
