...
import { calculateObv } from '@/domains/technicals/obv/service';
import prisma from '@/adapters/prisma/index';
import { analysisPrisma } from '@/adapters/prisma/analysisClient';

test('obv: 2330 歷史夠深，算得出一個累積值（可能正可能負，不釘死方向）', async () => {
  const result = await calculateObv({ companyId: '2330' });
...
test('obv: 只有一天資料時 OBV 應該是 0（沒有前一天可以比較漲跌）', async () => {
  const result = await calculateObv({ companyId: '1337', asOfDate: '2026-08-17' });
...
test('obv: 9999（查無資料的公司）回傳 not_applicable', async () => {
  const result = await calculateObv({ companyId: '9999' });
