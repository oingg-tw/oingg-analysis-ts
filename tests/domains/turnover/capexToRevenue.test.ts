...
import { calculateCapexToRevenue } from '@/domains/turnover/capexToRevenue/service';
import prisma from '@/adapters/prisma/index';
import { analysisPrisma } from '@/adapters/prisma/analysisClient';

// 對照 2330 115Q2 合併報表實測值。
test('capexToRevenue: 2330 115Q2 合併報表，指定季度', async () => {
  const result = await calculateCapexToRevenue({ companyId: '2330', year: '115', season: '2', dataType: '2', subsidiaryCompanyId: '' });
...
test('capexToRevenue: 2330 不指定 year/season 時自動抓最新一季，結果應該跟指定最新季度一致', async () => {
  const explicit = await calculateCapexToRevenue({ companyId: '2330', year: '115', season: '2', dataType: '2', subsidiaryCompanyId: '' });
  const auto = await calculateCapexToRevenue({ companyId: '2330', dataType: '2', subsidiaryCompanyId: '' });
...
test('capexToRevenue: 9999（查無資料的公司）自動抓最新一季應該回傳 year/season 為 null 的優雅降級結果', async () => {
  const result = await calculateCapexToRevenue({ companyId: '9999', dataType: '2', subsidiaryCompanyId: '' });
