...
import { findFilterCatalogProblems } from '@/api/bff/filter/filterCatalogCheck';
import { filterCatalog, type FilterCategory } from '@/api/bff/filter/filterCatalog';
...
describe('findFilterCatalogProblems', () => {
  ...
  test('model 名稱以 Curated 開頭時（數據中台的 curated 層原始資料副本）刻意不列進 catalog，不算漏加', () => {
    // 對應 CuratedMopsQuarterlyIncomeStatement 這類同步過來的原始資料副本，不是算出來的指標，
    // 即使有 Decimal 欄位（例如 eps）也不該被抓成「忘記加進 filterCatalog.ts」。
    const emptyCatalog: FilterCategory[] = [];
    const fakeSchema = `
      model CuratedMopsQuarterlyIncomeStatement {
        symbol String
        ...
        metrics: [
          {
            ...
            fields: [{ key: 'inventoryTurnoverTtm', name: '存貨周轉率', period: 'ttm', sort: 1 }],
          },
          {
            ...
            unit: 'ratio',
            modelKey: 'turnoverRatio',
            fields: [{ key: 'assetTurnoverTtm', name: '總資產周轉率', period: 'ttm', sort: 1 }],
          },
        ],
      },
    ];
  ...
  test('全市場單一值的 model（例如 equityRiskPremium）刻意不列進 catalog，不算漏加', () => {
    // 對應 EquityRiskPremiumResult——大盤股權風險溢酬全市場只有一個數字，不是「某支股票」的欄位，
    // 使用者定調不開放拿來篩選個股，所以 filterCatalog.ts 故意不列，這裡要確認檢查工具不會誤報。
    const emptyCatalog: FilterCategory[] = [];
    const fakeSchema = `
        ...
        metrics: [
          {
            ...
            path: '/guru/graham-number',
            unit: 'ratio',
            fields: [{ key: 'grahamNumber', name: '葛拉漢數', period: 'ttm', sort: 1 }],
          },
        ],
      },
    ];
