...
import { calculateGrahamNumber } from '@/domains/guru/grahamNumber/service';
...
test('grahamNumber: 2330 115Q2 合併報表，指定季度', async () => {
  const result = await calculateGrahamNumber({ companyId: '2330', year: '115', season: '2', dataType: '2', subsidiaryCompanyId: '' });
...
test('grahamNumber: 2330 不指定 year/season 時自動抓最新一季，結果應該跟指定最新季度一致', async () => {
  const explicit = await calculateGrahamNumber({ companyId: '2330', year: '115', season: '2', dataType: '2', subsidiaryCompanyId: '' });
  const auto = await calculateGrahamNumber({ companyId: '2330', dataType: '2', subsidiaryCompanyId: '' });
...
test('grahamNumber: 自動抓最新一季應該取資產負債表/損益表都有資料的交集，不是任一張表自己的最新一季', async () => {
  ...
  const auto = await calculateGrahamNumber({ companyId: '2887', dataType: '2', subsidiaryCompanyId: '' });
...
test('grahamNumber: 9999（查無資料的公司）自動抓最新一季應該回傳 year/season 為 null 的優雅降級結果', async () => {
  const result = await calculateGrahamNumber({ companyId: '9999', dataType: '2', subsidiaryCompanyId: '' });
