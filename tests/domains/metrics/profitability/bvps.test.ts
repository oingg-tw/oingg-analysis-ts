...
test('bvps: 2330 115Q2 合併報表，指定季度', async () => {
  const result = await calculateBvps({ companyId: '2330', year: '115', season: '2', dataType: '2', subsidiaryCompanyId: '' });
...
test('bvps: 2330 不指定 year/season 時自動抓最新一季，結果應該跟指定最新季度一致', async () => {
  const explicit = await calculateBvps({ companyId: '2330', year: '115', season: '2', dataType: '2', subsidiaryCompanyId: '' });
  const auto = await calculateBvps({ companyId: '2330', dataType: '2', subsidiaryCompanyId: '' });
...
test('bvps: 2887 只依賴資產負債表，自動抓最新一季應該直接是資產負債表自己的最新一季（115Q1），不受損益表落後影響', async () => {
  const auto = await calculateBvps({ companyId: '2887', dataType: '2', subsidiaryCompanyId: '' });
...
test('bvps: 9999（查無資料的公司）自動抓最新一季應該回傳 year/season 為 null 的優雅降級結果', async () => {
  const result = await calculateBvps({ companyId: '9999', dataType: '2', subsidiaryCompanyId: '' });
