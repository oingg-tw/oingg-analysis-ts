...
import { calculateRsi } from '@/domainBatch/metrics/technicals/rsi/service';
import { mopsExportPrisma } from '@/adapters/prisma/mopsExportClient';
import { analysisPrisma } from '@/adapters/prisma/analysisClient';

test('rsi: 2330 歷史夠深，三個窗口都算得出 0~100 之間的值', async () => {
  const result = await calculateRsi({ symbol: '2330' });
...
test('rsi: 資料量不足時回傳 null 並標成 no_data，不是拋錯', async () => {
  const result = await calculateRsi({ symbol: '1337' });
...
test('rsi: 9999（查無資料的公司）回傳 not_applicable', async () => {
  const result = await calculateRsi({ symbol: '9999' });
