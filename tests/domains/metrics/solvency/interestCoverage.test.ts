...
test('interestCoverage: 2330 不指定 year/season 時自動抓最新一季，結果應該跟指定最新季度一致', async () => {
  ...
  assert.equal(auto.interestCoverageQuarterly, explicit.interestCoverageQuarterly);
});

// 關鍵案例：interestCoverage 只需要損益表一張表——不寫死季度數字（mops 資料持續在補，2887 資產
// 負債表/損益表的落差會隨時間縮小甚至消失），改成直接拿 getLatestAvailableQuarter 對同一組 sources
...
test('interestCoverage: 9999（查無資料的公司）自動抓最新一季應該回傳 year/season 為 null 的優雅降級結果', async () => {
  ...
  assert.equal(result.interestCoverageQuarterly, null);
